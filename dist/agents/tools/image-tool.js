...
export function createImageTool(options) {
    ...
    return {
        ...
        execute: async (_toolCallId, args) => {
            ...
            let resolvedPath = isDataUrl ? null : resolvedPathInfo.resolved;
            // Fallback: when the resolved path doesn't exist, try media/inbound/<basename>
            // Models sometimes hallucinate paths (e.g. "workspace\file.png" instead of "media/inbound/file.png")
            if (resolvedPath && !isDataUrl && !path.isAbsolute(resolvedPath)) {
                try {
                    await fs.access(resolvedPath);
                }
                catch {
                    const fallback = path.join("media", "inbound", path.basename(resolvedPath));
                    try {
                        await fs.access(fallback);
                        resolvedPath = fallback;
                    }
                    catch {
                        // Keep original path; loadWebMedia will produce a clear error
                    }
                }
            }
