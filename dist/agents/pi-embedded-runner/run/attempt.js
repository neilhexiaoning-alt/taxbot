...
import { runImagePrompt, resolveImageModelConfigForTool } from "../../tools/image-tool.js";
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                try {
                    // When the primary model doesn't support images but images are attached,
                    // use the configured imageModel (e.g. qwen-vl-max) to extract text descriptions
                    // and prepend them to the prompt. This way the text-only model can still
                    // understand what the images contain.
                    const primaryModelHasVision = params.model.input?.includes("image") ?? false;
                    if (!primaryModelHasVision && params.images?.length > 0) {
                        const imageModelConfig = resolveImageModelConfigForTool({
                            cfg: params.config,
                            agentDir: agentDir,
                        });
                        if (imageModelConfig) {
                            log.debug(`Vision pre-processing: ${params.images.length} image(s) attached but model ${params.modelId} lacks vision. Using imageModel to extract text.`);
                            const descriptions = [];
                            for (let i = 0; i < params.images.length; i++) {
                                ...
                                try {
                                    const result = await runImagePrompt({
                                        ...
                                        agentDir: agentDir,
                                        imageModelConfig,
                                        prompt: "请仔细识别并描述这张图片中的所有内容。如果图片包含文字，请完整提取所有文字并保持原有结构和格式。如果包含表格，请用markdown表格格式呈现。如果包含图表或示意图，请详细描述其内容。",
                                        base64: img.data,
                                        mimeType: img.mimeType ?? "image/png",
                                    });
                                    if (result.text) {
                                        descriptions.push(`【图片${params.images.length > 1 ? (i + 1) : ""}内容识别结果】\n${result.text}`);
                                        log.debug(`Vision pre-processing: image ${i + 1} recognized, ${result.text.length} chars (model: ${result.provider}/${result.model})`);
                                    }
                                } catch (err) {
                                    log.warn(`Vision pre-processing: failed to recognize image ${i + 1}: ${err instanceof Error ? err.message : String(err)}`);
                                }
                            }
                            if (descriptions.length > 0) {
                                effectivePrompt = `${descriptions.join("\n\n")}\n\n${effectivePrompt}`;
                            }
                            // Clear images since they've been converted to text
