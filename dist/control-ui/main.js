...
`):""}async function U(e){if(!(!e.client||!e.connected)){e.chatLoading=!0,e.lastError=null;try{const n=await e.client.request("chat.history",{sessionKey:e.sessionKey,limit:200});e.chatMessages=Array.isArray(n.messages)?n.messages:[],e.chatThinkingLevel=n.thinkingLevel??null}catch(n){e.lastError=String(n)}finally{e.chatLoading=!1}}}function sa(e){const n=/^data:([^;]+);base64,(.+)$/.exec(e);return n?{mimeType:n[1],content:n[2]}:null}async function aa(e,n,t){if(!e.client||!e.connected)return null;const i=n.trim(),s=t&&t.length>0;if(!i&&!s)return null;const a=Date.now(),o=[];if(i&&o.push({type:"text",text:i}),s)for(const f of t)o.push({type:"image",source:{type:"base64",media_type:f.mimeType,data:f.dataUrl}});e.chatMessages=[...e.chatMessages,{role:"user",content:o,timestamp:a}],e.chatSending=!0,e.lastError=null;const r=nt();e.chatRunId=r,e.chatStream="",e.chatStreamStartedAt=a;const d=s?t.map(f=>{const y=sa(f.dataUrl);return y?{type:"image",mimeType:y.mimeType,content:y.content}:null}).filter(f=>f!==null):void 0;try{return await e.client.request("chat.send",{sessionKey:e.sessionKey,message:i,deliver:!1,idempotencyKey:r,attachments:d}),r}catch(f){const y=String(f);return e.chatRunId=null,e.chatStream=null,e.chatStreamStartedAt=null,e.lastError=y,e.chatMessages=[...e.chatMessages,{role:"assistant",content:[{type:"text",text:"Error: "+y}],timestamp:Date.now()}],null}finally{e.chatSending=!1}}async function la(e){if(!e.client||!e.connected)return!1;const n=e.chatRunId;try{return await e.client.request("chat.abort",n?{sessionKey:e.sessionKey,runId:n}:{sessionKey:e.sessionKey}),!0}catch(t){return e.lastError=String(t),!1}}function oa(e,n){if(!n||n.sessionKey!==e.sessionKey)return null;if(n.runId&&e.chatRunId&&n.runId!==e.chatRunId)return n.state==="final"?"final":null;if(n.state==="delta"){const t=Ze(n.message);if(typeof t=="string"){const i=e.chatStream??"";(!i||t.length>=i.length)&&(e.chatStream=t)}}else n.state==="final"||n.state==="aborted"?(e.chatStream=null,e.chatRunId=null,e.chatStreamStartedAt=null):n.state==="error"&&(e.chatStream=null,e.chatRunId=null,e.chatStreamStartedAt=null,e.lastError=n.errorMessage??"chat error");return n.state}const Lt=120;function It(e){return e.chatSending||!!e.chatRunId}function ra(e){const n=e.trim();if(!n)return!1;const t=n.toLowerCase();return t==="/stop"?!0:t==="stop"||t==="esc"||t==="abort"||t==="wait"||t==="exit"}function ca(e){const n=e.trim();if(!n)return!1;const t=n.toLowerCase();return t==="/new"||t==="/reset"?!0:t.startsWith("/new ")||t.startsWith("/reset ")}async function _t(e){e.connected&&(e.chatMessage="",await la(e))}function da(e,n,t,i){const s=n.trim(),a=!!(t&&t.length>0);!s&&!a||(e.chatQueue=[...e.chatQueue,{id:nt(),text:s,createdAt:Date.now(),attachments:a?t?.map(o=>({...o})):void 0,refreshSessions:i}])}async function Mt(e,n,t){Ie(e);const i=await aa(e,n,t?.attachments),s=!!i;return!s&&t?.previousDraft!=null&&(e.chatMessage=t.previousDraft),!s&&t?.previousAttachments&&(e.chatAttachments=t.previousAttachments),s&&St(e,e.sessionKey),s&&t?.restoreDraft&&t.previousDraft?.trim()&&(e.chatMessage=t.previousDraft),s&&t?.restoreAttachments&&t.previousAttachments?.length&&(e.chatAttachments=t.previousAttachments),re(e),s&&!e.chatRunId&&Et(e),s&&t?.refreshSessions&&i&&e.refreshSessionsAfterChat.add(i),s}async function Et(e){if(!e.connected||It(e))return;const[n,...t]=e.chatQueue;if(!n)return;e.chatQueue=t,await Mt(e,n.text,{attachments:n.attachments,refreshSessions:n.refreshSessions})||(e.chatQueue=[n,...e.chatQueue])}function ua(e,n){e.chatQueue=e.chatQueue.filter(t=>t.id!==n)}async function ga(e,n,t){if(!e.connected)return;const i=e.chatMessage,s=(n??e.chatMessage).trim(),a=e.chatAttachments??[],o=n==null?a:[],r=o.length>0;if(!s&&!r)return;if(ra(s)){await _t(e);return}const d=ca(s);if(n==null&&(e.chatMessage="",e.chatAttachments=[]),It(e)){da(e,s,o,d);return}await Mt(e,s,{previousDraft:n==null?i:void 0,restoreDraft:!!(n&&t?.restoreDraft),attachments:r?o:void 0,previousAttachments:n==null?a:void 0,restoreAttachments:!!(n&&t?.restoreDraft),refreshSessions:d})}async function Ft(e){await Promise.all([U(e),W(e,{activeMinutes:Lt}),Z(e)]),re(e)}const va=Et;function fa(e){const n=vt(e.sessionKey);return n?.agentId?n.agentId:e.hello?.snapshot?.sessionDefaults?.defaultAgentId?.trim()||"main"}function pa(e,n){const t=Ce(e),i=encodeURIComponent(n);return t?`${t}/avatar/${i}?meta=1`:`/avatar/${i}?meta=1`}async function Z(e){if(!e.connected){e.chatAvatarUrl=null;return}const n=fa(e);if(!n){e.chatAvatarUrl=null;return}e.chatAvatarUrl=null;const t=pa(e.basePath,n);try{const i=await fetch(t,{method:"GET"});if(!i.ok){e.chatAvatarUrl=null;return}const s=await i.json(),a=typeof s.avatarUrl=="string"?s.avatarUrl.trim():"";e.chatAvatarUrl=a||null}catch{e.chatAvatarUrl=null}}const ha={trace:!0,debug:!0,info:!0,warn:!0,error:!0,fatal:!0},ma={name:"",description:"",agentId:"",enabled:!0,scheduleKind:"every",scheduleAt:"",everyAmount:"30",everyUnit:"minutes",cronExpr:"0 7 * * *",cronTz:"",sessionTarget:"main",wakeMode:"next-heartbeat",payloadKind:"systemEvent",payloadText:"",deliver:!1,channel:"last",to:"",timeoutSeconds:"",postToMainPrefix:""},ya=50,ba=200,$a="Assistant";function In(e,n){if(typeof e!="string")return;const t=e.trim();if(t)return t.length<=n?t:t.slice(0,n)}function en(e){const n=In(e?.name,ya)??$a,t=In(e?.avatar??void 0,ba)??null;return{agentId:typeof e?.agentId=="string"&&e.agentId.trim()?e.agentId.trim():null,name:n,avatar:t}}function wa(){return en(typeof window>"u"?{}:{name:window.__OPENCLAW_ASSISTANT_NAME__,avatar:window.__OPENCLAW_ASSISTANT_AVATAR__})}async function Tt(e,n){if(!e.client||!e.connected)return;const t=e.sessionKey.trim(),i=t?{sessionKey:t}:{};try{const s=await e.client.request("agent.identity.get",i);if(!s)return;const a=en(s);e.assistantName=a.name,e.assistantAvatar=a.avatar,e.assistantAgentId=a.agentId??null}catch{}}function nn(e){return typeof e=="object"&&e!==null}function Sa(e){if(!nn(e))return null;const n=typeof e.id=="string"?e.id.trim():"",t=e.request;if(!n||!nn(t))return null;const i=typeof t.command=="string"?t.command.trim():"";if(!i)return null;const s=typeof e.createdAtMs=="number"?e.createdAtMs:0,a=typeof e.expiresAtMs=="number"?e.expiresAtMs:0;return!s||!a?null:{id:n,request:{command:i,cwd:typeof t.cwd=="string"?t.cwd:null,host:typeof t.host=="string"?t.host:null,security:typeof t.security=="string"?t.security:null,ask:typeof t.ask=="string"?t.ask:null,agentId:typeof t.agentId=="string"?t.agentId:null,resolvedPath:typeof t.resolvedPath=="string"?t.resolvedPath:null,sessionKey:typeof t.sessionKey=="string"?t.sessionKey:null},createdAtMs:s,expiresAtMs:a}}function xa(e){if(!nn(e))return null;const n=typeof e.id=="string"?e.id.trim():"";return n?{id:n,decision:typeof e.decision=="string"?e.decision:null,resolvedBy:typeof e.resolvedBy=="string"?e.resolvedBy:null,ts:typeof e.ts=="number"?e.ts:null}:null}function Rt(e){const n=Date.now();return e.filter(t=>t.expiresAtMs>n)}function ka(e,n){const t=Rt(e).filter(i=>i.id!==n.id);return t.push(n),t}function _n(e,n){return Rt(e).filter(t=>t.id!==n)}function Ke(e,n){const t=(e??"").trim(),i=n.mainSessionKey?.trim();if(!i)return t;if(!t)return i;const s=n.mainKey?.trim()||"main",a=n.defaultAgentId?.trim();return t==="main"||t===s||a&&(t===`agent:${a}:main`||t===`agent:${a}:${s}`)?i:t}function Aa(e,n){if(!n?.mainSessionKey)return;const t=Ke(e.sessionKey,n),i=Ke(e.settings.sessionKey,n),s=Ke(e.settings.lastActiveSessionKey,n),a=t||i||e.sessionKey,o={...e.settings,sessionKey:i||a,lastActiveSessionKey:s||a},r=o.sessionKey!==e.settings.sessionKey||o.lastActiveSessionKey!==e.settings.lastActiveSessionKey;a!==e.sessionKey&&(e.sessionKey=a),r&&H(e,o)}function Pt(e){e.lastError=null,e.hello=null,e.connected=!1,e.execApprovalQueue=[],e.execApprovalError=null,e.client?.stop(),e.client=new gi({url:e.settings.gatewayUrl,token:e.settings.token.trim()?e.settings.token:void 0,password:e.password.trim()?e.password:void 0,clientName:"openclaw-control-ui",mode:"webchat",onHello:n=>{e.connected=!0,e.lastError=null,e.hello=n,Ia(e,n),e.chatRunId=null,e.chatStream=null,e.chatStreamStartedAt=null,Ie(e),Tt(e),un(e),xe(e,{quiet:!0}),z(e,{quiet:!0}),hn(e)},onClose:({code:n,reason:t})=>{e.connected=!1,n!==1012&&(e.lastError=`disconnected (${n}): ${t||"no reason"}`)},onEvent:n=>Ca(e,n),onGap:({expected:n,received:t})=>{e.lastError=`event gap detected (expected seq ${n}, got ${t}); refresh recommended`}}),e.client.start()}function Ca(e,n){try{La(e,n)}catch(t){console.error("[gateway] handleGatewayEvent error:",n.event,t)}}function La(e,n){if(e.eventLogBuffer=[{ts:Date.now(),event:n.event,payload:n.payload},...e.eventLogBuffer].slice(0,250),e.tab==="debug"&&(e.eventLog=e.eventLogBuffer),n.event==="agent"){if(e.onboarding)return;Ys(e,n.payload);return}if(n.event==="chat"){const t=n.payload;t?.sessionKey&&St(e,t.sessionKey);const i=oa(e,t);if(i==="final"||i==="error"||i==="aborted"){Ie(e),va(e);const s=t?.runId;s&&e.refreshSessionsAfterChat.has(s)&&(e.refreshSessionsAfterChat.delete(s),i==="final"&&W(e,{activeMinutes:Lt}))}i==="final"&&U(e);return}if(n.event==="presence"){const t=n.payload;t?.presence&&Array.isArray(t.presence)&&(e.presenceEntries=t.presence,e.presenceError=null,e.presenceStatus=null);return}if(n.event==="cron"&&e.tab==="cron"&&be(e),(n.event==="device.pair.requested"||n.event==="device.pair.resolved")&&z(e,{quiet:!0}),n.event==="exec.approval.requested"){const t=Sa(n.payload);if(t){e.execApprovalQueue=ka(e.execApprovalQueue,t),e.execApprovalError=null;const i=Math.max(0,t.expiresAtMs-Date.now()+500);window.setTimeout(()=>{e.execApprovalQueue=_n(e.execApprovalQueue,t.id)},i)}return}if(n.event==="exec.approval.resolved"){const t=xa(n.payload);t&&(e.execApprovalQueue=_n(e.execApprovalQueue,t.id))}}function Ia(e,n){const t=n.snapshot;t?.presence&&Array.isArray(t.presence)&&(e.presenceEntries=t.presence),t?.health&&(e.debugHealth=t.health),t?.sessionDefaults&&Aa(e,t.sessionDefaults)}function _a(e){e.basePath=Rs(),Es(e),Ds(e,!0),Ps(e),Bs(e),window.addEventListener("popstate",e.popStateHandler),Pt(e),Xi(e),e.tab==="logs"&&on(e),e.tab==="debug"&&cn(e)}function Ma(e){Vi(e)}function Ea(e){window.removeEventListener("popstate",e.popStateHandler),Zi(e),rn(e),dn(e),Ns(e),e.topbarObserver?.disconnect(),e.topbarObserver=null}function Fa(e,n){if(e.tab==="chat"&&(n.has("chatMessages")||n.has("chatToolMessages")||n.has("chatStream")||n.has("chatLoading")||n.has("tab"))){const t=n.has("tab"),i=n.has("chatLoading")&&n.get("chatLoading")===!0&&!e.chatLoading;re(e,t||i||!e.chatHasAutoScrolled)}e.tab==="logs"&&(n.has("logsEntries")||n.has("logsAutoFollow")||n.has("tab"))&&e.logsAutoFollow&&e.logsAtBottom&&ft(e,n.has("tab")||n.has("logsAutoFollow"))}const Mn=(e,n,t)=>{const i=new Map;for(let s=n;s<=t;s++)i.set(e[s],s);return i},Bt=tt(class extends it{constructor(e){if(super(e),e.type!==st.CHILD)throw Error("repeat() can only be used in text expressions")}dt(e,n,t){let i;t===void 0?t=n:n!==void 0&&(i=n);const s=[],a=[];let o=0;for(const r of e)s[o]=i?i(r,o):o,a[o]=t(r,o),o++;return{values:a,keys:s}}render(e,n,t){return this.dt(e,n,t).values}update(e,[n,t,i]){const s=vi(e),{values:a,keys:o}=this.dt(n,t,i);if(!Array.isArray(s))return this.ut=o,a;const r=this.ut??=[],d=[];let f,y,g=0,c=s.length-1,u=0,b=a.length-1;for(;g<=c&&u<=b;)if(s[g]===null)g++;else if(s[c]===null)c--;else if(r[g]===o[u])d[u]=V(s[g],a[u]),g++,u++;else if(r[c]===o[b])d[b]=V(s[c],a[b]),c--,b--;else if(r[g]===o[b])d[b]=V(s[g],a[b]),X(e,d[b+1],s[g]),g++,b--;else if(r[c]===o[u])d[u]=V(s[c],a[u]),X(e,s[g],s[c]),c--,u++;else if(f===void 0&&(f=Mn(o,u,b),y=Mn(r,g,c)),f.has(r[g]))if(f.has(r[c])){const $=y.get(o[u]),w=$!==void 0?s[$]:null;if(w===null){const S=X(e,s[g]);V(S,a[u]),d[u]=S}else d[u]=V(w,a[u]),X(e,s[g],w),s[$]=null;u++}else Pe(s[c]),c--;else Pe(s[g]),g++;for(;u<=b;){const $=X(e,d[b+1]);V($,a[u]),d[u++]=$}for(;g<=c;){const $=s[g++];$!==null&&Pe($)}return this.ut=o,pi(e,d),fi}}),M={messageSquare:l`
  ...
  `};function Ta(e,n){const t=fn(n,e.basePath);return l`
    <a
      href=${t}
  ...
  `}function Ra(e){const n=Pa(e.hello,e.sessionsResult),t=Ba(e.sessionKey,e.sessionsResult,n),i=e.onboarding,s=e.onboarding,a=e.onboarding?!1:e.settings.chatShowThinking,o=e.onboarding?!0:e.settings.chatFocusMode,r=l`
    <svg
      width="18"
      height="18"
      viewBox="0 0 24 24"
  ...
  `;return l`
    <div class="chat-controls">
      <label class="field chat-controls__session">
        ...
        >
          ${Bt(t,f=>f.key,f=>l`<option value=${f.key}>
                ${f.displayName??f.key}
              </option>`)}
        </select>
      </label>
  ...
  `}function Oa(){return l`
    <svg class="theme-icon" viewBox="0 0 24 24" aria-hidden="true">
      <path
        d="M20.985 12.486a9 9 0 1 1-9.473-9.472c.405-.022.617.46.402.803a6 6 0 0 0 8.268 8.268c.344-.215.825-.004.803.401"
      ></path>
    </svg>
  `}function Ua(){return l`
    <svg class="theme-icon" viewBox="0 0 24 24" aria-hidden="true">
      <rect width="20" height="14" x="2" y="3" rx="2"></rect>
      <line x1="8" x2="16" y1="21" y2="21"></line>
      <line x1="12" x2="12" y1="17" y2="21"></line>
    </svg>
  `}function Nt(e,n){if(!e)return e;const i=e.files.some(s=>s.name===n.name)?e.files.map(s=>s.name===n.name?n:s):[...e.files,n];return{...e,files:i}}async function Ue(e,n){if(!(!e.client||!e.connected||e.agentFilesLoading)){e.agentFilesLoading=!0,e.agentFilesError=null;try{const t=await e.client.request("agents.files.list",{agentId:n});t&&(e.agentFilesList=t,e.agentFileActive&&!t.files.some(i=>i.name===e.agentFileActive)&&(e.agentFileActive=null))}catch(t){e.agentFilesError=String(t)}finally{e.agentFilesLoading=!1}}}async function ja(e,n,t){if(!(!e.client||!e.connected||e.agentFilesLoading)&&!Object.hasOwn(e.agentFileContents,t)){e.agentFilesLoading=!0,e.agentFilesError=null;try{const i=await e.client.request("agents.files.get",{agentId:n,name:t});if(i?.file){const s=i.file.content??"";e.agentFilesList=Nt(e.agentFilesList,i.file),e.agentFileContents={...e.agentFileContents,[t]:s},Object.hasOwn(e.agentFileDrafts,t)||(e.agentFileDrafts={...e.agentFileDrafts,[t]:s})}}catch(i){e.agentFilesError=String(i)}finally{e.agentFilesLoading=!1}}}async function Ha(e,n,t,i){if(!(!e.client||!e.connected||e.agentFileSaving)){e.agentFileSaving=!0,e.agentFilesError=null;try{const s=await e.client.request("agents.files.set",{agentId:n,name:t,content:i});s?.file&&(e.agentFilesList=Nt(e.agentFilesList,s.file),e.agentFileContents={...e.agentFileContents,[t]:i},e.agentFileDrafts={...e.agentFileDrafts,[t]:i})}catch(s){e.agentFilesError=String(s)}finally{e.agentFileSaving=!1}}}const za={bash:"exec","apply-patch":"apply_patch"},Va={"group:memory":["memory_search","memory_get"],"group:web":["web_search","web_fetch"],"group:fs":["read","write","edit","apply_patch"],"group:runtime":["exec","process"],"group:sessions":["sessions_list","sessions_history","sessions_send","sessions_spawn","session_status"],"group:ui":["browser","canvas"],"group:automation":["cron","gateway"],"group:messaging":["message"],"group:nodes":["nodes"],"group:openclaw":["browser","canvas","nodes","cron","message","gateway","agents_list","sessions_list","sessions_history","sessions_send","sessions_spawn","session_status","memory_search","memory_get","web_search","web_fetch","image"]},qa={minimal:{allow:["session_status"]},coding:{allow:["group:fs","group:runtime","group:sessions","group:memory","image"]},messaging:{allow:["group:messaging","sessions_list","sessions_history","sessions_send","session_status"]},full:{}};function P(e){const n=e.trim().toLowerCase();return za[n]??n}function Ga(e){return e?e.map(P).filter(Boolean):[]}function Wa(e){const n=Ga(e),t=[];for(const i of n){const s=Va[i];if(s){t.push(...s);continue}t.push(i)}return Array.from(new Set(t))}function Qa(e){if(!e)return;const n=qa[e];if(n&&!(!n.allow&&!n.deny))return{allow:n.allow?[...n.allow]:void 0,deny:n.deny?[...n.deny]:void 0}}function Ja(e){const n=e.host??"unknown",t=e.ip?`(${e.ip})`:"",i=e.mode??"",s=e.version??"";return`${n} ${t} ${i} ${s}`.trim()}function Ya(e){const n=e.ts??null;return n?_(n):"n/a"}function mn(e){return e?`${ae(e)} (${_(e)})`:"n/a"}function Xa(e){if(e.totalTokens==null)return"n/a";const n=e.totalTokens??0,t=e.contextTokens??0;return t?`${n} / ${t}`:String(n)}function Za(e){if(e==null)return"";try{return JSON.stringify(e,null,2)}catch{return String(e)}}function Dt(e){const n=e.state??{},t=n.nextRunAtMs?ae(n.nextRunAtMs):"n/a",i=n.lastRunAtMs?ae(n.lastRunAtMs):"n/a";return`${n.lastStatus??"n/a"} · next ${t} · last ${i}`}function Kt(e){const n=e.schedule;return n.kind==="at"?`At ${ae(n.atMs)}`:n.kind==="every"?`Every ${at(n.everyMs)}`:`Cron ${n.expr}${n.tz?` (${n.tz})`:""}`}function Ot(e){const n=e.payload;return n.kind==="systemEvent"?`System: ${n.text}`:`Agent: ${n.message}`}const En=[{id:"fs",label:"Files",tools:[{id:"read",label:"read",description:"Read file contents"},{id:"write",label:"write",description:"Create or overwrite files"},{id:"edit",label:"edit",description:"Make precise edits"},{id:"apply_patch",label:"apply_patch",description:"Patch files (OpenAI)"}]},{id:"runtime",label:"Runtime",tools:[{id:"exec",label:"exec",description:"Run shell commands"},{id:"process",label:"process",description:"Manage background processes"}]},{id:"web",label:"Web",tools:[{id:"web_search",label:"web_search",description:"Search the web"},{id:"web_fetch",label:"web_fetch",description:"Fetch web content"}]},{id:"memory",label:"Memory",tools:[{id:"memory_search",label:"memory_search",description:"Semantic search"},{id:"memory_get",label:"memory_get",description:"Read memory files"}]},{id:"sessions",label:"Sessions",tools:[{id:"sessions_list",label:"sessions_list",description:"List sessions"},{id:"sessions_history",label:"sessions_history",description:"Session history"},{id:"sessions_send",label:"sessions_send",description:"Send to session"},{id:"sessions_spawn",label:"sessions_spawn",description:"Spawn sub-agent"},{id:"session_status",label:"session_status",description:"Session status"}]},{id:"ui",label:"UI",tools:[{id:"browser",label:"browser",description:"Control web browser"},{id:"canvas",label:"canvas",description:"Control canvases"}]},{id:"messaging",label:"Messaging",tools:[{id:"message",label:"message",description:"Send messages"}]},{id:"automation",label:"Automation",tools:[{id:"cron",label:"cron",description:"Schedule tasks"},{id:"gateway",label:"gateway",description:"Gateway control"}]},{id:"nodes",label:"Nodes",tools:[{id:"nodes",label:"nodes",description:"Nodes + devices"}]},{id:"agents",label:"Agents",tools:[{id:"agents_list",label:"agents_list",description:"List agents"}]},{id:"media",label:"Media",tools:[{id:"image",label:"image",description:"Image understanding"}]}],el=[{id:"minimal",label:"Minimal"},{id:"coding",label:"Coding"},{id:"messaging",label:"Messaging"},{id:"full",label:"Full"}];function tn(e){return e.name?.trim()||e.identity?.name?.trim()||e.id}function fe(e){const n=e.trim();if(!n||n.length>16)return!1;let t=!1;for(let i=0;i<n.length;i+=1)if(n.charCodeAt(i)>127){t=!0;break}return!(!t||n.includes("://")||n.includes("/")||n.includes("."))}function _e(e,n){const t=n?.emoji?.trim();if(t&&fe(t))return t;const i=e.identity?.emoji?.trim();if(i&&fe(i))return i;const s=n?.avatar?.trim();if(s&&fe(s))return s;const a=e.identity?.avatar?.trim();return a&&fe(a)?a:""}function Ut(e,n){return n&&e===n?"default":null}function nl(e){if(e==null||!Number.isFinite(e))return"-";if(e<1024)return`${e} B`;const n=["KB","MB","GB","TB"];let t=e/1024,i=0;for(;t>=1024&&i<n.length-1;)t/=1024,i+=1;return`${t.toFixed(t<10?1:0)} ${n[i]}`}function Me(e,n){const t=e;return{entry:(t?.agents?.list??[]).find(a=>a?.id===n),defaults:t?.agents?.defaults,globalTools:t?.tools}}function jt(e,n,t,i,s){const a=Me(n,e.id),r=(t&&t.agentId===e.id?t.workspace:null)||a.entry?.workspace||a.defaults?.workspace||"default",d=a.entry?.model?ne(a.entry?.model):ne(a.defaults?.model),f=s?.name?.trim()||e.identity?.name?.trim()||e.name?.trim()||a.entry?.name||e.id,y=_e(e,s)||"-",g=Array.isArray(a.entry?.skills)?a.entry?.skills:null,c=g?.length??null;return{workspace:r,model:d,identityName:f,identityEmoji:y,skillsLabel:g?`${c} selected`:"all skills",isDefault:!!(i&&e.id===i)}}function ne(e){if(!e)return"-";if(typeof e=="string")return e.trim()||"-";if(typeof e=="object"&&e){const n=e,t=n.primary?.trim();if(t){const i=Array.isArray(n.fallbacks)?n.fallbacks.length:0;return i>0?`${t} (+${i} fallback)`:t}}return"-"}function Fn(e){const n=e.match(/^(.+) \(\+\d+ fallback\)$/);return n?n[1]:e}function Tn(e){if(!e)return null;if(typeof e=="string")return e.trim()||null;if(typeof e=="object"&&e){const n=e;return(typeof n.primary=="string"?n.primary:typeof n.model=="string"?n.model:typeof n.id=="string"?n.id:typeof n.value=="string"?n.value:null)?.trim()||null}return null}function tl(e){if(!e||typeof e=="string")return null;if(typeof e=="object"&&e){const n=e,t=Array.isArray(n.fallbacks)?n.fallbacks:Array.isArray(n.fallback)?n.fallback:null;return t?t.filter(i=>typeof i=="string"):null}return null}function il(e){return e.split(",").map(n=>n.trim()).filter(Boolean)}function sl(e){const t=e?.agents?.defaults?.models;if(!t||typeof t!="object")return[];const i=[];for(const[s,a]of Object.entries(t)){const o=s.trim();if(!o)continue;const r=a&&typeof a=="object"&&"alias"in a&&typeof a.alias=="string"?a.alias?.trim():void 0,d=r&&r!==o?`${r} (${o})`:o;i.push({value:o,label:d})}return i}function al(e,n){const t=sl(e),i=n?t.some(s=>s.value===n):!1;return n&&!i&&t.unshift({value:n,label:`Current (${n})`}),t.length===0?l`
      <option value="" disabled>No configured models</option>
    `:t.map(s=>l`<option value=${s.value}>${s.label}</option>`)}function ll(e){const n=P(e);if(!n)return{kind:"exact",value:""};if(n==="*")return{kind:"all"};if(!n.includes("*"))return{kind:"exact",value:n};const t=n.replace(/[.*+?^${}()|[\\]\\]/g,"\\$&");return{kind:"regex",value:new RegExp(`^${t.replaceAll("\\*",".*")}$`)}}function sn(e){return Array.isArray(e)?Wa(e).map(ll).filter(n=>n.kind!=="exact"||n.value.length>0):[]}function te(e,n){for(const t of n)if(t.kind==="all"||t.kind==="exact"&&e===t.value||t.kind==="regex"&&t.value.test(e))return!0;return!1}function ol(e,n){if(!n)return!0;const t=P(e),i=sn(n.deny);if(te(t,i))return!1;const s=sn(n.allow);return!!(s.length===0||te(t,s)||t==="apply_patch"&&te("exec",s))}function Rn(e,n){if(!Array.isArray(n)||n.length===0)return!1;const t=P(e),i=sn(n);return!!(te(t,i)||t==="apply_patch"&&te("exec",i))}function rl(e){const n=e.agentsList?.agents??[],t=e.agentsList?.defaultId??null,i=e.selectedAgentId??t??n[0]?.id??null,s=i?n.find(a=>a.id===i)??null:null;return l`
    <div class="agents-layout">
      <section class="card agents-sidebar">
        ...
        <div class="agent-list" style="margin-top: 12px;">
          ${n.length===0?l`
                ...
                `:n.map(a=>{const o=Ut(a.id,t),r=_e(a,e.agentIdentityById[a.id]??null);return l`
                    <button
                      type="button"
      ...
      <section class="agents-main">
        ${s?l`
              ${cl(s,t,e.agentIdentityById[s.id]??null)}
              ${dl(e.activePanel,a=>e.onSelectPanel(a))}
              ${e.activePanel==="overview"?ul({agent:s,defaultId:t,configForm:e.configForm,agentFilesList:e.agentFilesList,agentIdentity:e.agentIdentityById[s.id]??null,agentIdentityError:e.agentIdentityError,agentIdentityLoading:e.agentIdentityLoading,configLoading:e.configLoading,configSaving:e.configSaving,configDirty:e.configDirty,onConfigReload:e.onConfigReload,onConfigSave:e.onConfigSave,onModelChange:e.onModelChange,onModelFallbacksChange:e.onModelFallbacksChange}):v}
              ${e.activePanel==="files"?wl({agentId:s.id,agentFilesList:e.agentFilesList,agentFilesLoading:e.agentFilesLoading,agentFilesError:e.agentFilesError,agentFileActive:e.agentFileActive,agentFileContents:e.agentFileContents,agentFileDrafts:e.agentFileDrafts,agentFileSaving:e.agentFileSaving,onLoadFiles:e.onLoadFiles,onSelectFile:e.onSelectFile,onFileDraftChange:e.onFileDraftChange,onFileReset:e.onFileReset,onFileSave:e.onFileSave}):v}
              ${e.activePanel==="tools"?xl({agentId:s.id,configForm:e.configForm,configLoading:e.configLoading,configSaving:e.configSaving,configDirty:e.configDirty,onProfileChange:e.onToolsProfileChange,onOverridesChange:e.onToolsOverridesChange,onConfigReload:e.onConfigReload,onConfigSave:e.onConfigSave}):v}
              ${e.activePanel==="skills"?Al({agentId:s.id,report:e.agentSkillsReport,loading:e.agentSkillsLoading,error:e.agentSkillsError,activeAgentId:e.agentSkillsAgentId,configForm:e.configForm,configLoading:e.configLoading,configSaving:e.configSaving,configDirty:e.configDirty,filter:e.skillsFilter,onFilterChange:e.onSkillsFilterChange,onRefresh:e.onSkillsRefresh,onToggle:e.onAgentSkillToggle,onClear:e.onAgentSkillsClear,onDisableAll:e.onAgentSkillsDisableAll,onConfigReload:e.onConfigReload,onConfigSave:e.onConfigSave}):v}
              ${e.activePanel==="channels"?bl({agent:s,defaultId:t,configForm:e.configForm,agentFilesList:e.agentFilesList,agentIdentity:e.agentIdentityById[s.id]??null,snapshot:e.channelsSnapshot,loading:e.channelsLoading,error:e.channelsError,lastSuccess:e.channelsLastSuccess,onRefresh:e.onChannelsRefresh}):v}
              ${e.activePanel==="cron"?$l({agent:s,defaultId:t,configForm:e.configForm,agentFilesList:e.agentFilesList,agentIdentity:e.agentIdentityById[s.id]??null,jobs:e.cronJobs,status:e.cronStatus,loading:e.cronLoading,error:e.cronError,onRefresh:e.onCronRefresh}):v}
            `:l`
                <div class="card">
  ...
  `}function cl(e,n,t){const i=Ut(e.id,n),s=tn(e),a=e.identity?.theme?.trim()||"Agent workspace and routing.",o=_e(e,t);return l`
  ...
  `}function ul(e){const{agent:n,configForm:t,agentFilesList:i,agentIdentity:s,agentIdentityLoading:a,agentIdentityError:o,configLoading:r,configSaving:d,configDirty:f,onConfigReload:y,onConfigSave:g,onModelChange:c,onModelFallbacksChange:u}=e,b=Me(t,n.id),w=(i&&i.agentId===n.id?i.workspace:null)||b.entry?.workspace||b.defaults?.workspace||"default",S=b.entry?.model?ne(b.entry?.model):ne(b.defaults?.model),x=ne(b.defaults?.model),A=Tn(b.entry?.model)||(S!=="-"?Fn(S):null),L=Tn(b.defaults?.model)||(x!=="-"?Fn(x):null),C=A??L??null,k=tl(b.entry?.model),I=k?k.join(", "):"",ue=s?.name?.trim()||n.identity?.name?.trim()||n.name?.trim()||b.entry?.name||"-",si=_e(n,s)||"-",Sn=Array.isArray(b.entry?.skills)?b.entry?.skills:null,ai=Sn?.length??null,xn=a?"Loading…":o?"Unavailable":"",li=!!(e.defaultId&&n.id===e.defaultId);return l`
    <section class="card">
      ...
      <div class="agents-overview-grid" style="margin-top: 16px;">
        ...
        <div class="agent-kv">
          ...
          <div>${Sn?`${ai} selected`:"all skills"}</div>
        </div>
      </div>
        ...
        <div class="row" style="gap: 12px; flex-wrap: wrap;">
          ...
          <label class="field" style="min-width: 260px; flex: 1;">
            ...
            <input
              ...
              @input=${Te=>u(n.id,il(Te.target.value))}
            />
          </label>
        </div>
  ...
  `}function gl(e,n){const t=e.channelMeta?.find(i=>i.id===n);return t?.label?t.label:e.channelLabels?.[n]??n}function vl(e){if(!e)return[];const n=new Set;for(const s of e.channelOrder??[])n.add(s);for(const s of e.channelMeta??[])n.add(s.id);for(const s of Object.keys(e.channelAccounts??{}))n.add(s);const t=[],i=e.channelOrder?.length?e.channelOrder:Array.from(n);for(const s of i)n.has(s)&&(t.push(s),n.delete(s));for(const s of n)t.push(s);return t.map(s=>({id:s,label:gl(e,s),accounts:e.channelAccounts?.[s]??[]}))}const fl=["groupPolicy","streamMode","dmPolicy"];function pl(e,n){if(!e)return null;const i=(e.channels??{})[n];if(i&&typeof i=="object")return i;const s=e[n];return s&&typeof s=="object"?s:null}function hl(e){if(e==null)return"n/a";if(typeof e=="string"||typeof e=="number"||typeof e=="boolean")return String(e);try{return JSON.stringify(e)}catch{return"n/a"}}function ml(e,n){const t=pl(e,n);return t?fl.flatMap(i=>i in t?[{label:i,value:hl(t[i])}]:[]):[]}function yl(e){let n=0,t=0,i=0;for(const s of e){const a=s.probe&&typeof s.probe=="object"&&"ok"in s.probe?!!s.probe.ok:!1;(s.connected===!0||s.running===!0||a)&&(n+=1),s.configured&&(t+=1),s.enabled&&(i+=1)}return{total:e.length,connected:n,configured:t,enabled:i}}function bl(e){const n=jt(e.agent,e.configForm,e.agentFilesList,e.defaultId,e.agentIdentity),t=vl(e.snapshot),i=e.lastSuccess?_(e.lastSuccess):"never";return l`
    <section class="grid grid-cols-2">
      ${Ht(n,"Workspace, identity, and model configuration.")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
        ...
        ${t.length===0?l`
              ...
              <div class="list" style="margin-top: 16px;">
                ${t.map(s=>{const a=yl(s.accounts),o=a.total?`${a.connected}/${a.total} connected`:"no accounts",r=a.configured?`${a.configured} configured`:"not configured",d=a.total?`${a.enabled} enabled`:"disabled",f=ml(e.configForm,s.id);return l`
                    <div class="list-item">
                      <div class="list-main">
  ...
  `}function $l(e){const n=jt(e.agent,e.configForm,e.agentFilesList,e.defaultId,e.agentIdentity),t=e.jobs.filter(i=>i.agentId===e.agent.id);return l`
    <section class="grid grid-cols-2">
      ${Ht(n,"Workspace and scheduling targets.")}
      <section class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
              ...
              <div class="list" style="margin-top: 16px;">
                ${t.map(i=>l`
                  <div class="list-item">
                    ...
                    <div class="list-meta">
                      <div class="mono">${Dt(i)}</div>
                      <div class="muted">${Ot(i)}</div>
                    </div>
                  </div>
                `)}
              </div>
  ...
  `}function wl(e){const n=e.agentFilesList?.agentId===e.agentId?e.agentFilesList:null,t=n?.files??[],i=e.agentFileActive??null,s=i?t.find(d=>d.name===i)??null:null,a=i?e.agentFileContents[i]??"":"",o=i?e.agentFileDrafts[i]??a:"",r=i?o!==a:!1;return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
              ...
              <div class="agent-files-grid" style="margin-top: 16px;">
                <div class="agent-files-list">
                  ${t.length===0?l`
                        ...
                        `:t.map(d=>Sl(d,i,()=>e.onSelectFile(d.name)))}
                </div>
  ...
  `}function Sl(e,n,t){const i=e.missing?"Missing":`${nl(e.size)} · ${_(e.updatedAtMs??null)}`;return l`
    <button
      type="button"
  ...
  `}function xl(e){const n=Me(e.configForm,e.agentId),t=n.entry?.tools??{},i=n.globalTools??{},s=t.profile??i.profile??"full",a=t.profile?"agent override":i.profile?"global default":"default",o=Array.isArray(t.allow)&&t.allow.length>0,r=Array.isArray(i.allow)&&i.allow.length>0,d=!!e.configForm&&!e.configLoading&&!e.configSaving&&!o,f=o?[]:Array.isArray(t.alsoAllow)?t.alsoAllow:[],y=o?[]:Array.isArray(t.deny)?t.deny:[],g=o?{allow:t.allow??[],deny:t.deny??[]}:Qa(s)??void 0,c=En.flatMap(S=>S.tools.map(x=>x.id)),u=S=>{const x=ol(S,g),A=Rn(S,f),L=Rn(S,y);return{allowed:(x||A)&&!L,baseAllowed:x,denied:L}},b=c.filter(S=>u(S).allowed).length,$=(S,x)=>{const A=new Set(f.map(I=>P(I)).filter(I=>I.length>0)),L=new Set(y.map(I=>P(I)).filter(I=>I.length>0)),C=u(S).baseAllowed,k=P(S);x?(L.delete(k),C||A.add(k)):(A.delete(k),L.add(k)),e.onOverridesChange(e.agentId,[...A],[...L])},w=S=>{const x=new Set(f.map(L=>P(L)).filter(L=>L.length>0)),A=new Set(y.map(L=>P(L)).filter(L=>L.length>0));for(const L of c){const C=u(L).baseAllowed,k=P(L);S?(A.delete(k),C||x.add(k)):(x.delete(k),A.add(k))}e.onOverridesChange(e.agentId,[...x],[...A])};return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
  ...
  `}const je=[{id:"workspace",label:"Workspace Skills",sources:["openclaw-workspace"]},{id:"built-in",label:"Built-in Skills",sources:["openclaw-bundled"]},{id:"installed",label:"Installed Skills",sources:["openclaw-managed"]},{id:"extra",label:"Extra Skills",sources:["openclaw-extra"]}];function kl(e){const n=new Map;for(const s of je)n.set(s.id,{id:s.id,label:s.label,skills:[]});const t={id:"other",label:"Other Skills",skills:[]};for(const s of e){const a=je.find(o=>o.sources.includes(s.source));a?n.get(a.id)?.skills.push(s):t.skills.push(s)}const i=je.map(s=>n.get(s.id)).filter(s=>!!(s&&s.skills.length>0));return t.skills.length>0&&i.push(t),i}function Al(e){const n=!!e.configForm&&!e.configLoading&&!e.configSaving,t=Me(e.configForm,e.agentId),i=Array.isArray(t.entry?.skills)?t.entry?.skills:void 0,s=new Set((i??[]).map(u=>u.trim()).filter(Boolean)),a=i!==void 0,o=!!(e.report&&e.activeAgentId===e.agentId),r=o?e.report?.skills??[]:[],d=e.filter.trim().toLowerCase(),f=d?r.filter(u=>[u.name,u.description,u.source].join(" ").toLowerCase().includes(d)):r,y=kl(f),g=a?r.filter(u=>s.has(u.name)).length:r.length,c=r.length;return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
      ...
      ${f.length===0?l`
            ...
            `:l`
              <div class="agent-skills-groups" style="margin-top: 16px;">
                ${y.map(u=>Cl(u,{agentId:e.agentId,allowSet:s,usingAllowlist:a,editable:n,onToggle:e.onToggle}))}
              </div>
            `}
    </section>
  `}function Cl(e,n){const t=e.id==="workspace"||e.id==="built-in";return l`
    <details class="agent-skills-group" ?open=${!t}>
      ...
      <div class="list skills-grid">
        ${e.skills.map(i=>Ll(i,{agentId:n.agentId,allowSet:n.allowSet,usingAllowlist:n.usingAllowlist,editable:n.editable,onToggle:n.onToggle}))}
      </div>
    </details>
  `}function Ll(e,n){const t=n.usingAllowlist?n.allowSet.has(e.name):!0,i=[...e.missing.bins.map(a=>`bin:${a}`),...e.missing.env.map(a=>`env:${a}`),...e.missing.config.map(a=>`config:${a}`),...e.missing.os.map(a=>`os:${a}`)],s=[];return e.disabled&&s.push("disabled"),e.blockedByAllowlist&&s.push("blocked by allowlist"),l`
  ...
  `}function B(e){if(e)return Array.isArray(e.type)?e.type.filter(t=>t!=="null")[0]??e.type[0]:e.type}function zt(e){if(!e)return"";if(e.default!==void 0)return e.default;switch(B(e)){case"object":return{};case"array":return[];case"boolean":return!1;case"number":case"integer":return 0;case"string":return"";default:return""}}function Ee(e){return e.filter(n=>typeof n=="string").join(".")}function T(e,n){const t=Ee(e),i=n[t];if(i)return i;const s=t.split(".");for(const[a,o]of Object.entries(n)){if(!a.includes("*"))continue;const r=a.split(".");if(r.length!==s.length)continue;let d=!0;for(let f=0;f<s.length;f+=1)if(r[f]!=="*"&&r[f]!==s[f]){d=!1;break}if(d)return o}}function K(e){return e.replace(/_/g," ").replace(/([a-z0-9])([A-Z])/g,"$1 $2").replace(/\s+/g," ").replace(/^./,n=>n.toUpperCase())}function Il(e){const n=Ee(e).toLowerCase();return n.includes("token")||n.includes("password")||n.includes("secret")||n.includes("apikey")||n.endsWith("key")}const _l=new Set(["title","description","default","nullable"]);function Ml(e){return Object.keys(e??{}).filter(t=>!_l.has(t)).length===0}function El(e){if(e===void 0)return"";try{return JSON.stringify(e,null,2)??""}catch{return""}}const oe={chevronDown:l`
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
  ...
  `};function D(e){const{schema:n,value:t,path:i,hints:s,unsupported:a,disabled:o,onPatch:r}=e,d=e.showLabel??!0,f=B(n),y=T(i,s),g=y?.label??n.title??K(String(i.at(-1))),c=y?.help??n.description,u=Ee(i);if(a.has(u))return l`<div class="cfg-field cfg-field--error">
      ...
      `}if(x&&S.length>5)return Bn({...e,options:S,value:t??n.default});const A=new Set($.map(C=>B(C)).filter(Boolean)),L=new Set([...A].map(C=>C==="integer"?"number":C));if([...L].every(C=>["string","number","boolean"].includes(C))){const C=L.has("string"),k=L.has("number");if(L.has("boolean")&&L.size===1)return D({...e,schema:{...n,type:"boolean",anyOf:void 0,oneOf:void 0}});if(C||k)return Pn({...e,inputType:k&&!C?"number":"text"})}}if(n.enum){const b=n.enum;if(b.length<=5){const $=t??n.default;return l`
        <div class="cfg-field">
          ${d?l`<label class="cfg-field__label">${g}</label>`:v}
          ${c?l`<div class="cfg-field__help">${c}</div>`:v}
          <div class="cfg-segmented">
      ...
      `}return Bn({...e,options:b,value:t??n.default})}if(f==="object")return Tl(e);if(f==="array")return Rl(e);if(f==="boolean"){const b=typeof t=="boolean"?t:typeof n.default=="boolean"?n.default:!1;return l`
    ...
    `}return f==="number"||f==="integer"?Fl(e):f==="string"?Pn({...e,inputType:"text"}):l`
    <div class="cfg-field cfg-field--error">
      <div class="cfg-field__label">${g}</div>
      <div class="cfg-field__error">Unsupported type: ${f}. Use Raw mode.</div>
    </div>
  `}function Pn(e){const{schema:n,value:t,path:i,hints:s,disabled:a,onPatch:o,inputType:r}=e,d=e.showLabel??!0,f=T(i,s),y=f?.label??n.title??K(String(i.at(-1))),g=f?.help??n.description,c=f?.sensitive??Il(i),u=f?.placeholder??(c?"••••":n.default!==void 0?`Default: ${String(n.default)}`:""),b=t??"";return l`
    <div class="cfg-field">
  ...
  `}function Fl(e){const{schema:n,value:t,path:i,hints:s,disabled:a,onPatch:o}=e,r=e.showLabel??!0,d=T(i,s),f=d?.label??n.title??K(String(i.at(-1))),y=d?.help??n.description,g=t??n.default??"",c=typeof g=="number"?g:0;return l`
    <div class="cfg-field">
  ...
  `}function Bn(e){const{schema:n,value:t,path:i,hints:s,disabled:a,options:o,onPatch:r}=e,d=e.showLabel??!0,f=T(i,s),y=f?.label??n.title??K(String(i.at(-1))),g=f?.help??n.description,c=t??n.default,u=o.findIndex($=>$===c||String($)===String(c)),b="__unset__";return l`
    <div class="cfg-field">
      ${d?l`<label class="cfg-field__label">${y}</label>`:v}
      ${g?l`<div class="cfg-field__help">${g}</div>`:v}
      <select
  ...
  `}function Tl(e){const{schema:n,value:t,path:i,hints:s,unsupported:a,disabled:o,onPatch:r}=e,d=T(i,s),f=d?.label??n.title??K(String(i.at(-1))),y=d?.help??n.description,g=t??n.default,c=g&&typeof g=="object"&&!Array.isArray(g)?g:{},u=n.properties??{},$=Object.entries(u).toSorted((A,L)=>{const C=T([...i,A[0]],s)?.order??0,k=T([...i,L[0]],s)?.order??0;return C!==k?C-k:A[0].localeCompare(L[0])}),w=new Set(Object.keys(u)),S=n.additionalProperties,x=!!S&&typeof S=="object";return i.length===1?l`
      <div class="cfg-fields">
        ...
        ${x?Nn({schema:S,value:c,path:i,hints:s,unsupported:a,disabled:o,reservedKeys:w,onPatch:r}):v}
      </div>
    `:l`
  ...
  `}function Rl(e){const{schema:n,value:t,path:i,hints:s,unsupported:a,disabled:o,onPatch:r}=e,d=e.showLabel??!0,f=T(i,s),y=f?.label??n.title??K(String(i.at(-1))),g=f?.help??n.description,c=Array.isArray(n.items)?n.items[0]:n.items;if(!c)return l`
      <div class="cfg-field cfg-field--error">
  ...
  `}function Nn(e){const{schema:n,value:t,path:i,hints:s,unsupported:a,disabled:o,reservedKeys:r,onPatch:d}=e,f=Ml(n),y=Object.entries(t??{}).filter(([g])=>!r.has(g));return l`
    <div class="cfg-map">
      <div class="cfg-map__header">
        ...
        <button
          ...
          class="cfg-map__add"
          ?disabled=${o}
          @click=${()=>{const g={...t};let c=1,u=`custom-${c}`;for(;u in g;)c+=1,u=`custom-${c}`;g[u]=f?{}:zt(n),d(i,g)}}
        >
      ...
      ${y.length===0?l`
        ...
        <div class="cfg-map__items">
          ${y.map(([g,c])=>{const u=[...i,g],b=El(c);return l`
      ...
      ${y?(()=>{const{sectionKey:g,subsectionKey:c,schema:u}=y,b=T([g,c],e.uiHints),$=b?.label??u.title??K(c),w=b?.help??u.description??"",S=t[g],x=S&&typeof S=="object"?S[c]:void 0,A=`config-section-${g}-${c}`;return l`
              <section class="config-section-card" id=${A}>
                <div class="config-section-card__header">
                  <span class="config-section-card__icon">${Kn(g)}</span>
  ...
  `}const Nl=new Set(["title","description","default","nullable"]);function Dl(e){return Object.keys(e??{}).filter(t=>!Nl.has(t)).length===0}function Vt(e){const n=e.filter(s=>s!=null),t=n.length!==e.length,i=[];for(const s of n)i.some(a=>Object.is(a,s))||i.push(s);return{enumValues:i,nullable:t}}function qt(e){return!e||typeof e!="object"?{schema:null,unsupportedPaths:["<root>"]}:ie(e,[])}function ie(e,n){const t=new Set,i={...e},s=Ee(n)||"<root>";if(e.anyOf||e.oneOf||e.allOf){const r=Kl(e,n);return r||{schema:e,unsupportedPaths:[s]}}const a=Array.isArray(e.type)&&e.type.includes("null"),o=B(e)??(e.properties||e.additionalProperties?"object":void 0);if(i.type=o??e.type,i.nullable=a||e.nullable,i.enum){const{enumValues:r,nullable:d}=Vt(i.enum);i.enum=r,d&&(i.nullable=!0),r.length===0&&t.add(s)}if(o==="object"){const r=e.properties??{},d={};for(const[f,y]of Object.entries(r)){const g=ie(y,[...n,f]);g.schema&&(d[f]=g.schema);for(const c of g.unsupportedPaths)t.add(c)}if(i.properties=d,e.additionalProperties===!0)t.add(s);else if(e.additionalProperties===!1)i.additionalProperties=!1;else if(e.additionalProperties&&typeof e.additionalProperties=="object"&&!Dl(e.additionalProperties)){const f=ie(e.additionalProperties,[...n,"*"]);i.additionalProperties=f.schema??e.additionalProperties,f.unsupportedPaths.length>0&&t.add(s)}}else if(o==="array"){const r=Array.isArray(e.items)?e.items[0]:e.items;if(!r)t.add(s);else{const d=ie(r,[...n,"*"]);i.items=d.schema??r,d.unsupportedPaths.length>0&&t.add(s)}}else o!=="string"&&o!=="number"&&o!=="integer"&&o!=="boolean"&&!i.enum&&t.add(s);return{schema:i,unsupportedPaths:Array.from(t)}}function Kl(e,n){if(e.allOf)return null;const t=e.anyOf??e.oneOf;if(!t)return null;const i=[],s=[];let a=!1;for(const r of t){if(!r||typeof r!="object")return null;if(Array.isArray(r.enum)){const{enumValues:d,nullable:f}=Vt(r.enum);i.push(...d),f&&(a=!0);continue}if("const"in r){if(r.const==null){a=!0;continue}i.push(r.const);continue}if(B(r)==="null"){a=!0;continue}s.push(r)}if(i.length>0&&s.length===0){const r=[];for(const d of i)r.some(f=>Object.is(f,d))||r.push(d);return{schema:{...e,enum:r,nullable:a,anyOf:void 0,oneOf:void 0,allOf:void 0},unsupportedPaths:[]}}if(s.length===1){const r=ie(s[0],n);return r.schema&&(r.schema.nullable=a||r.schema.nullable),r}const o=new Set(["string","number","integer","boolean"]);return s.length>0&&i.length===0&&s.every(r=>r.type&&o.has(String(r.type)))?{schema:{...e,nullable:a},unsupportedPaths:[]}:null}function Ol(e,n){let t=e;for(const i of n){if(!t)return null;const s=B(t);if(s==="object"){const a=t.properties??{};if(typeof i=="string"&&a[i]){t=a[i];continue}const o=t.additionalProperties;if(typeof i=="string"&&o&&typeof o=="object"){t=o;continue}return null}if(s==="array"){if(typeof i!="number")return null;t=(Array.isArray(t.items)?t.items[0]:t.items)??null;continue}return null}return t}function Ul(e,n){const i=(e.channels??{})[n],s=e[n];return(i&&typeof i=="object"?i:null)??(s&&typeof s=="object"?s:null)??{}}const jl=["groupPolicy","streamMode","dmPolicy"];function Hl(e){if(e==null)return"n/a";if(typeof e=="string"||typeof e=="number"||typeof e=="boolean")return String(e);try{return JSON.stringify(e)}catch{return"n/a"}}function zl(e){const n=jl.flatMap(t=>t in e?[[t,e[t]]]:[]);return n.length===0?null:l`
  ...
  `}function Vl(e){const n=qt(e.schema),t=n.schema;if(!t)return l`
      <div class="callout danger">Schema unavailable. Use Raw.</div>
    `;const i=Ol(t,["channels",e.channelId]);if(!i)return l`
      <div class="callout danger">Channel config schema unavailable.</div>
    `;const s=e.configValue??{},a=Ul(s,e.channelId);return l`
  ...
  `}function O(e){const{channelId:n,props:t}=e,i=t.configSaving||t.configSchemaLoading;return l`
    <div style="margin-top: 16px;">
      ${t.configSchemaLoading?l`
            ...
            `:Vl({channelId:n,configValue:t.configForm,schema:t.configSchema,uiHints:t.configUiHints,disabled:i,onPatch:t.onConfigPatch})}
  ...
  `}function ql(e){const{props:n,discord:t,accountCountLabel:i}=e;return l`
    <div class="card">
      ...
      <div class="status-list" style="margin-top: 16px;">
        ...
        <div>
          ...
          <span>${t?.lastStartAt?_(t.lastStartAt):"n/a"}</span>
        </div>
        <div>
          <span class="label">Last probe</span>
          <span>${t?.lastProbeAt?_(t.lastProbeAt):"n/a"}</span>
        </div>
      </div>
  ...
  `}function Gl(e){const{props:n,googleChat:t,accountCountLabel:i}=e;return l`
    <div class="card">
  ...
  `}function Wl(e){const{props:n,imessage:t,accountCountLabel:i}=e;return l`
    <div class="card">
  ...
  `}function On(e){return e?e.length<=20?e:`${e.slice(0,8)}...${e.slice(-8)}`:"n/a"}function Ql(e){const{props:n,nostr:t,nostrAccounts:i,accountCountLabel:s,profileFormState:a,profileFormCallbacks:o,onEditProfile:r}=e,d=i[0],f=t?.configured??d?.configured??!1,y=t?.running??d?.running??!1,g=t?.publicKey??d?.publicKey,c=t?.lastStartAt??d?.lastStartAt??null,u=t?.lastError??d?.lastError??null,b=i.length>1,$=a!=null,w=x=>{const A=x.publicKey,L=x.profile,C=L?.displayName??L?.name??x.name??x.accountId;return l`
      <div class="account-card">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            ...
            <span class="monospace" title="${A??""}">${On(A)}</span>
          </div>
          <div>
            <span class="label">Last inbound</span>
            <span>${x.lastInboundAt?_(x.lastInboundAt):"n/a"}</span>
          </div>
          ${x.lastError?l`
    ...
    `},S=()=>{if($&&o)return Ii({state:a,callbacks:o,accountId:i[0]?.accountId??"default"});const x=d?.profile??t?.profile,{name:A,displayName:L,about:C,picture:k,nip05:I}=x??{},ue=A||L||C||k||I;return l`
  ...
  `}function Jl(e){if(!e&&e!==0)return"n/a";const n=Math.round(e/1e3);if(n<60)return`${n}s`;const t=Math.round(n/60);return t<60?`${t}m`:`${Math.round(t/60)}h`}function Yl(e,n){const t=n.snapshot,i=t?.channels;if(!t||!i)return!1;const s=i[e],a=typeof s?.configured=="boolean"&&s.configured,o=typeof s?.running=="boolean"&&s.running,r=typeof s?.connected=="boolean"&&s.connected,f=(t.channelAccounts?.[e]??[]).some(y=>y.configured||y.running||y.connected);return a||o||r||f}function Xl(e,n){return n?.[e]?.length??0}function Gt(e,n){const t=Xl(e,n);return t<2?v:l`<div class="account-count">Accounts (${t})</div>`}function Zl(e){const{props:n,signal:t,accountCountLabel:i}=e;return l`
    <div class="card">
  ...
  `}function eo(e){const{props:n,slack:t,accountCountLabel:i}=e;return l`
    <div class="card">
  ...
  `}function no(e){const{props:n,telegram:t,telegramAccounts:i,accountCountLabel:s}=e,a=i.length>1,o=r=>{const f=r.probe?.bot?.username,y=r.name||r.accountId;return l`
      <div class="account-card">
        <div class="account-card-header">
        ...
        <div class="status-list account-card-status">
          ...
          <div>
            <span class="label">Last inbound</span>
            <span>${r.lastInboundAt?_(r.lastInboundAt):"n/a"}</span>
          </div>
          ${r.lastError?l`
  ...
  `}function to(e){const{props:n,whatsapp:t,accountCountLabel:i}=e;return l`
    <div class="card">
      ...
      <div class="status-list" style="margin-top: 16px;">
        ...
        <div>
          ...
          <span>
            ${t?.lastConnectedAt?_(t.lastConnectedAt):"n/a"}
          </span>
        </div>
        <div>
          <span class="label">Last message</span>
          <span>
            ${t?.lastMessageAt?_(t.lastMessageAt):"n/a"}
          </span>
        </div>
        <div>
          <span class="label">Auth age</span>
          <span>
            ${t?.authAgeMs!=null?Jl(t.authAgeMs):"n/a"}
          </span>
        </div>
      </div>
  ...
  `}function io(e){const n=e.snapshot?.channels,t=n?.whatsapp??void 0,i=n?.telegram??void 0,s=n?.discord??null;n?.googlechat;const a=n?.slack??null,o=n?.signal??null,r=n?.imessage??null,d=n?.nostr??null,y=so(e.snapshot).map((g,c)=>({key:g,enabled:Yl(g,e),order:c})).toSorted((g,c)=>g.enabled!==c.enabled?g.enabled?-1:1:g.order-c.order);return l`
    <section class="grid grid-cols-2">
      ${y.map(g=>ao(g.key,e,{whatsapp:t,telegram:i,discord:s,slack:a,signal:o,imessage:r,nostr:d,channelAccounts:e.snapshot?.channelAccounts??null}))}
    </section>
    ...
    <section class="card" style="margin-top: 18px;">
      <div class="row" style="justify-content: space-between;">
        ...
        <div class="muted">${e.lastSuccessAt?_(e.lastSuccessAt):"n/a"}</div>
      </div>
...
${e.snapshot?JSON.stringify(e.snapshot,null,2):"No snapshot yet."}
  ...
  `}function so(e){return e?.channelMeta?.length?e.channelMeta.map(n=>n.id):e?.channelOrder?.length?e.channelOrder:["whatsapp","telegram","discord","googlechat","slack","signal","imessage","nostr"]}function ao(e,n,t){const i=Gt(e,t.channelAccounts);switch(e){case"whatsapp":return to({props:n,whatsapp:t.whatsapp,accountCountLabel:i});case"telegram":return no({props:n,telegram:t.telegram,telegramAccounts:t.channelAccounts?.telegram??[],accountCountLabel:i});case"discord":return ql({props:n,discord:t.discord,accountCountLabel:i});case"googlechat":return Gl({props:n,accountCountLabel:i});case"slack":return eo({props:n,slack:t.slack,accountCountLabel:i});case"signal":return Zl({props:n,signal:t.signal,accountCountLabel:i});case"imessage":return Wl({props:n,imessage:t.imessage,accountCountLabel:i});case"nostr":{const s=t.channelAccounts?.nostr??[],a=s[0],o=a?.accountId??"default",r=a?.profile??null,d=n.nostrProfileAccountId===o?n.nostrProfileFormState:null,f=d?{onFieldChange:n.onNostrProfileFieldChange,onSave:n.onNostrProfileSave,onImport:n.onNostrProfileImport,onCancel:n.onNostrProfileCancel,onToggleAdvanced:n.onNostrProfileToggleAdvanced}:null;return Ql({props:n,nostr:t.nostr,nostrAccounts:s,accountCountLabel:i,profileFormState:d,profileFormCallbacks:f,onEditProfile:()=>n.onNostrProfileEdit(o,r)})}default:return lo(e,n,t.channelAccounts??{})}}function lo(e,n,t){const i=ro(n.snapshot,e),s=n.snapshot?.channels?.[e],a=typeof s?.configured=="boolean"?s.configured:void 0,o=typeof s?.running=="boolean"?s.running:void 0,r=typeof s?.connected=="boolean"?s.connected:void 0,d=typeof s?.lastError=="string"?s.lastError:void 0,f=t[e]??[],y=Gt(e,t);return l`
    <div class="card">
  ...
  `}function oo(e){return e?.channelMeta?.length?Object.fromEntries(e.channelMeta.map(n=>[n.id,n])):{}}function ro(e,n){return oo(e)[n]?.label??e?.channelLabels?.[n]??n}const co=600*1e3;function Wt(e){return e.lastInboundAt?Date.now()-e.lastInboundAt<co:!1}function uo(e){return e.running?"Yes":Wt(e)?"Active":"No"}function go(e){return e.connected===!0?"Yes":e.connected===!1?"No":Wt(e)?"Active":"n/a"}function vo(e){const n=uo(e),t=go(e);return l`
    <div class="account-card">
      <div class="account-card-header">
      ...
      <div class="status-list account-card-status">
        ...
        <div>
          <span class="label">Last inbound</span>
          <span>${e.lastInboundAt?_(e.lastInboundAt):"n/a"}</span>
        </div>
        ${e.lastError?l`
              <div class="account-card-error">
                ${e.lastError}
  ...
  `}const se=(e,n)=>{const t=e._$AN;if(t===void 0)return!1;for(const i of t)i._$AO?.(n,!1),se(i,n);return!0},$e=e=>{let n,t;do{if((n=e._$AM)===void 0)break;t=n._$AN,t.delete(e),e=n}while(t?.size===0)},Qt=e=>{for(let n;n=e._$AM;e=n){let t=n._$AN;if(t===void 0)n._$AN=t=new Set;else if(t.has(e))break;t.add(e),ho(n)}};function fo(e){this._$AN!==void 0?($e(this),this._$AM=e,Qt(this)):this._$AM=e}function po(e,n=!1,t=0){const i=this._$AH,s=this._$AN;if(s!==void 0&&s.size!==0)if(n)if(Array.isArray(i))for(let a=t;a<i.length;a++)se(i[a],!1),$e(i[a]);else i!=null&&(se(i,!1),$e(i));else se(this,e)}const ho=e=>{e.type==st.CHILD&&(e._$AP??=po,e._$AQ??=fo)};class mo extends it{constructor(){super(...arguments),this._$AN=void 0}_$AT(n,t,i){super._$AT(n,t,i),Qt(this),this.isConnected=n._$AU}_$AO(n,t=!0){n!==this.isConnected&&(this.isConnected=n,n?this.reconnected?.():this.disconnected?.()),t&&(se(this,n),$e(this))}setValue(n){if(hi(this._$Ct))this._$Ct._$AI(n,this);else{const t=[...this._$Ct._$AH];t[this._$Ci]=n,this._$Ct._$AI(t,this,0)}}disconnected(){}reconnected(){}}const He=new WeakMap,yo=tt(class extends mo{render(e){return v}update(e,[n]){const t=n!==this.G;return t&&this.G!==void 0&&this.rt(void 0),(t||this.lt!==this.ct)&&(this.G=n,this.ht=e.options?.host,this.rt(this.ct=e.element)),v}rt(e){if(this.isConnected||(e=void 0),typeof this.G=="function"){const n=this.ht??globalThis;let t=He.get(n);t===void 0&&(t=new WeakMap,He.set(n,t)),t.get(this.G)!==void 0&&this.G.call(this.ht,void 0),t.set(this.G,e),e!==void 0&&this.G.call(this.ht,e)}else this.G.value=e}get lt(){return typeof this.G=="function"?He.get(this.ht??globalThis)?.get(this.G):this.G?.value}disconnected(){this.lt===this.ct&&this.rt(void 0)}reconnected(){this.rt(this.ct)}}),bo=1500,$o=2e3,Jt="Copy as markdown",wo="Copied",So="Copy failed";async function xo(e){if(!e)return!1;try{return await navigator.clipboard.writeText(e),!0}catch{return!1}}function pe(e,n){e.title=n,e.setAttribute("aria-label",n)}function ko(e){const n=e.label??Jt;return l`
    <button
      class="chat-copy-btn"
      type="button"
      title=${n}
      aria-label=${n}
      @click=${async t=>{const i=t.currentTarget;if(!i||i.dataset.copying==="1")return;i.dataset.copying="1",i.setAttribute("aria-busy","true"),i.disabled=!0;const s=await xo(e.text());if(i.isConnected){if(delete i.dataset.copying,i.removeAttribute("aria-busy"),i.disabled=!1,!s){i.dataset.error="1",pe(i,So),window.setTimeout(()=>{i.isConnected&&(delete i.dataset.error,pe(i,n))},$o);return}i.dataset.copied="1",pe(i,wo),window.setTimeout(()=>{i.isConnected&&(delete i.dataset.copied,pe(i,n))},bo)}}}
    >
  ...
  `}function Ao(e){return ko({text:()=>e,label:Jt})}function Yt(e){const n=e;let t=typeof n.role=="string"?n.role:"unknown";const i=typeof n.toolCallId=="string"||typeof n.tool_call_id=="string",s=n.content,a=Array.isArray(s)?s:null,o=Array.isArray(a)&&a.some(g=>{const c=g,u=(typeof c.type=="string"?c.type:"").toLowerCase();return u==="toolresult"||u==="tool_result"}),r=typeof n.toolName=="string"||typeof n.tool_name=="string";(i||o||r)&&(t="toolResult");let d=[];typeof n.content=="string"?d=[{type:"text",text:n.content}]:Array.isArray(n.content)?d=n.content.map(g=>({type:g.type||"text",text:g.text,name:g.name,args:g.args||g.arguments})):typeof n.text=="string"&&(d=[{type:"text",text:n.text}]);const f=typeof n.timestamp=="number"?n.timestamp:Date.now(),y=typeof n.id=="string"?n.id:void 0;return{role:t,content:d,timestamp:f,id:y}}function bn(e){const n=e.toLowerCase();return e==="user"||e==="User"?e:e==="assistant"?"assistant":e==="system"?"system":n==="toolresult"||n==="tool_result"||n==="tool"||n==="function"?"tool":e}function Xt(e){const n=e,t=typeof n.role=="string"?n.role.toLowerCase():"";return t==="toolresult"||t==="tool_result"}const Co={icon:"puzzle",detailKeys:["command","path","url","targetUrl","targetId","ref","element","node","nodeId","id","requestId","to","channelId","guildId","userId","name","query","pattern","messageId"]},Lo={bash:{icon:"wrench",title:"Bash",detailKeys:["command"]},process:{icon:"wrench",title:"Process",detailKeys:["sessionId"]},read:{icon:"fileText",title:"Read",detailKeys:["path"]},write:{icon:"edit",title:"Write",detailKeys:["path"]},edit:{icon:"penLine",title:"Edit",detailKeys:["path"]},attach:{icon:"paperclip",title:"Attach",detailKeys:["path","url","fileName"]},browser:{icon:"globe",title:"Browser",actions:{status:{label:"status"},start:{label:"start"},stop:{label:"stop"},tabs:{label:"tabs"},open:{label:"open",detailKeys:["targetUrl"]},focus:{label:"focus",detailKeys:["targetId"]},close:{label:"close",detailKeys:["targetId"]},snapshot:{label:"snapshot",detailKeys:["targetUrl","targetId","ref","element","format"]},screenshot:{label:"screenshot",detailKeys:["targetUrl","targetId","ref","element"]},navigate:{label:"navigate",detailKeys:["targetUrl","targetId"]},console:{label:"console",detailKeys:["level","targetId"]},pdf:{label:"pdf",detailKeys:["targetId"]},upload:{label:"upload",detailKeys:["paths","ref","inputRef","element","targetId"]},dialog:{label:"dialog",detailKeys:["accept","promptText","targetId"]},act:{label:"act",detailKeys:["request.kind","request.ref","request.selector","request.text","request.value"]}}},canvas:{icon:"image",title:"Canvas",actions:{present:{label:"present",detailKeys:["target","node","nodeId"]},hide:{label:"hide",detailKeys:["node","nodeId"]},navigate:{label:"navigate",detailKeys:["url","node","nodeId"]},eval:{label:"eval",detailKeys:["javaScript","node","nodeId"]},snapshot:{label:"snapshot",detailKeys:["format","node","nodeId"]},a2ui_push:{label:"A2UI push",detailKeys:["jsonlPath","node","nodeId"]},a2ui_reset:{label:"A2UI reset",detailKeys:["node","nodeId"]}}},nodes:{icon:"smartphone",title:"Nodes",actions:{status:{label:"status"},describe:{label:"describe",detailKeys:["node","nodeId"]},pending:{label:"pending"},approve:{label:"approve",detailKeys:["requestId"]},reject:{label:"reject",detailKeys:["requestId"]},notify:{label:"notify",detailKeys:["node","nodeId","title","body"]},camera_snap:{label:"camera snap",detailKeys:["node","nodeId","facing","deviceId"]},camera_list:{label:"camera list",detailKeys:["node","nodeId"]},camera_clip:{label:"camera clip",detailKeys:["node","nodeId","facing","duration","durationMs"]},screen_record:{label:"screen record",detailKeys:["node","nodeId","duration","durationMs","fps","screenIndex"]}}},cron:{icon:"loader",title:"Cron",actions:{status:{label:"status"},list:{label:"list"},add:{label:"add",detailKeys:["job.name","job.id","job.schedule","job.cron"]},update:{label:"update",detailKeys:["id"]},remove:{label:"remove",detailKeys:["id"]},run:{label:"run",detailKeys:["id"]},runs:{label:"runs",detailKeys:["id"]},wake:{label:"wake",detailKeys:["text","mode"]}}},gateway:{icon:"plug",title:"Gateway",actions:{restart:{label:"restart",detailKeys:["reason","delayMs"]},"config.get":{label:"config get"},"config.schema":{label:"config schema"},"config.apply":{label:"config apply",detailKeys:["restartDelayMs"]},"update.run":{label:"update run",detailKeys:["restartDelayMs"]}}},whatsapp_login:{icon:"circle",title:"WhatsApp Login",actions:{start:{label:"start"},wait:{label:"wait"}}},discord:{icon:"messageSquare",title:"Discord",actions:{react:{label:"react",detailKeys:["channelId","messageId","emoji"]},reactions:{label:"reactions",detailKeys:["channelId","messageId"]},sticker:{label:"sticker",detailKeys:["to","stickerIds"]},poll:{label:"poll",detailKeys:["question","to"]},permissions:{label:"permissions",detailKeys:["channelId"]},readMessages:{label:"read messages",detailKeys:["channelId","limit"]},sendMessage:{label:"send",detailKeys:["to","content"]},editMessage:{label:"edit",detailKeys:["channelId","messageId"]},deleteMessage:{label:"delete",detailKeys:["channelId","messageId"]},threadCreate:{label:"thread create",detailKeys:["channelId","name"]},threadList:{label:"thread list",detailKeys:["guildId","channelId"]},threadReply:{label:"thread reply",detailKeys:["channelId","content"]},pinMessage:{label:"pin",detailKeys:["channelId","messageId"]},unpinMessage:{label:"unpin",detailKeys:["channelId","messageId"]},listPins:{label:"list pins",detailKeys:["channelId"]},searchMessages:{label:"search",detailKeys:["guildId","content"]},memberInfo:{label:"member",detailKeys:["guildId","userId"]},roleInfo:{label:"roles",detailKeys:["guildId"]},emojiList:{label:"emoji list",detailKeys:["guildId"]},roleAdd:{label:"role add",detailKeys:["guildId","userId","roleId"]},roleRemove:{label:"role remove",detailKeys:["guildId","userId","roleId"]},channelInfo:{label:"channel",detailKeys:["channelId"]},channelList:{label:"channels",detailKeys:["guildId"]},voiceStatus:{label:"voice",detailKeys:["guildId","userId"]},eventList:{label:"events",detailKeys:["guildId"]},eventCreate:{label:"event create",detailKeys:["guildId","name"]},timeout:{label:"timeout",detailKeys:["guildId","userId"]},kick:{label:"kick",detailKeys:["guildId","userId"]},ban:{label:"ban",detailKeys:["guildId","userId"]}}},slack:{icon:"messageSquare",title:"Slack",actions:{react:{label:"react",detailKeys:["channelId","messageId","emoji"]},reactions:{label:"reactions",detailKeys:["channelId","messageId"]},sendMessage:{label:"send",detailKeys:["to","content"]},editMessage:{label:"edit",detailKeys:["channelId","messageId"]},deleteMessage:{label:"delete",detailKeys:["channelId","messageId"]},readMessages:{label:"read messages",detailKeys:["channelId","limit"]},pinMessage:{label:"pin",detailKeys:["channelId","messageId"]},unpinMessage:{label:"unpin",detailKeys:["channelId","messageId"]},listPins:{label:"list pins",detailKeys:["channelId"]},memberInfo:{label:"member",detailKeys:["userId"]},emojiList:{label:"emoji list"}}}},Io={fallback:Co,tools:Lo},Zt=Io,Un=Zt.fallback??{icon:"puzzle"},_o=Zt.tools??{};function Mo(e){return(e??"tool").trim()}function Eo(e){const n=e.replace(/_/g," ").trim();return n?n.split(/\s+/).map(t=>t.length<=2&&t.toUpperCase()===t?t:`${t.at(0)?.toUpperCase()??""}${t.slice(1)}`).join(" "):"Tool"}function Fo(e){const n=e?.trim();if(n)return n.replace(/_/g," ")}function ei(e){if(e!=null){if(typeof e=="string"){const n=e.trim();if(!n)return;const t=n.split(/\r?\n/)[0]?.trim()??"";return t?t.length>160?`${t.slice(0,157)}…`:t:void 0}if(typeof e=="number"||typeof e=="boolean")return String(e);if(Array.isArray(e)){const n=e.map(i=>ei(i)).filter(i=>!!i);if(n.length===0)return;const t=n.slice(0,3).join(", ");return n.length>3?`${t}…`:t}}}function To(e,n){if(!e||typeof e!="object")return;let t=e;for(const i of n.split(".")){if(!i||!t||typeof t!="object")return;t=t[i]}return t}function Ro(e,n){for(const t of n){const i=To(e,t),s=ei(i);if(s)return s}}function Po(e){if(!e||typeof e!="object")return;const n=e,t=typeof n.path=="string"?n.path:void 0;if(!t)return;const i=typeof n.offset=="number"?n.offset:void 0,s=typeof n.limit=="number"?n.limit:void 0;return i!==void 0&&s!==void 0?`${t}:${i}-${i+s}`:t}function Bo(e){if(!e||typeof e!="object")return;const n=e;return typeof n.path=="string"?n.path:void 0}function No(e,n){if(!(!e||!n))return e.actions?.[n]??void 0}function Do(e){const n=Mo(e.name),t=n.toLowerCase(),i=_o[t],s=i?.icon??Un.icon??"puzzle",a=i?.title??Eo(n),o=i?.label??n,r=e.args&&typeof e.args=="object"?e.args.action:void 0,d=typeof r=="string"?r.trim():void 0,f=No(i,d),y=Fo(f?.label??d);let g;t==="read"&&(g=Po(e.args)),!g&&(t==="write"||t==="edit"||t==="attach")&&(g=Bo(e.args));const c=f?.detailKeys??i?.detailKeys??Un.detailKeys??[];return!g&&c.length>0&&(g=Ro(e.args,c)),!g&&e.meta&&(g=e.meta),g&&(g=Oo(g)),{name:n,icon:s,title:a,label:o,verb:y,detail:g}}function Ko(e){const n=[];if(e.verb&&n.push(e.verb),e.detail&&n.push(e.detail),n.length!==0)return n.join(" · ")}function Oo(e){return e&&e.replace(/\/Users\/[^/]+/g,"~").replace(/\/home\/[^/]+/g,"~")}const Uo=80,jo=2,jn=100;function Ho(e){const n=e.trim();if(n.startsWith("{")||n.startsWith("["))try{const t=JSON.parse(n);return"```json\n"+JSON.stringify(t,null,2)+"\n```"}catch{}return e}function zo(e){const n=e.split(`
`),t=n.slice(0,jo),i=t.join(`
`);return i.length>jn?i.slice(0,jn)+"…":t.length<n.length?i+"…":i}function Vo(e){const n=e,t=qo(n.content),i=[];for(const s of t){const a=(typeof s.type=="string"?s.type:"").toLowerCase();(["toolcall","tool_call","tooluse","tool_use"].includes(a)||typeof s.name=="string"&&s.arguments!=null)&&i.push({kind:"call",name:s.name??"tool",args:Go(s.arguments??s.args)})}for(const s of t){const a=(typeof s.type=="string"?s.type:"").toLowerCase();if(a!=="toolresult"&&a!=="tool_result")continue;const o=Wo(s),r=typeof s.name=="string"?s.name:"tool";i.push({kind:"result",name:r,text:o})}if(Xt(e)&&!i.some(s=>s.kind==="result")){const s=typeof n.toolName=="string"&&n.toolName||typeof n.tool_name=="string"&&n.tool_name||"tool",a=Ct(e)??void 0;i.push({kind:"result",name:s,text:a})}return i}function Hn(e,n){const t=Do({name:e.name,args:e.args}),i=Ko(t),s=!!e.text?.trim(),a=!!n,o=a?()=>{if(s){n(Ho(e.text));return}const g=`## ${t.label}

${i?`**Command:** \`${i}\`

`:""}*No output — tool completed successfully.*`;n(g)}:void 0,r=s&&(e.text?.length??0)<=Uo,d=s&&!r,f=s&&r,y=!s;return l`
    <div
    ...
    >
      ...
      ${y?l`
            ...
            `:v}
      ${d?l`<div class="chat-tool-card__preview mono">${zo(e.text)}</div>`:v}
      ${f?l`<div class="chat-tool-card__inline mono">${e.text}</div>`:v}
    </div>
  `}function qo(e){return Array.isArray(e)?e.filter(Boolean):[]}function Go(e){if(typeof e!="string")return e;const n=e.trim();if(!n||!n.startsWith("{")&&!n.startsWith("["))return e;try{return JSON.parse(n)}catch{return e}}function Wo(e){if(typeof e.text=="string")return e.text;if(typeof e.content=="string")return e.content}function Qo(e){const t=e.content,i=[];if(Array.isArray(t))for(const s of t){if(typeof s!="object"||s===null)continue;const a=s;if(a.type==="image"){const o=a.source;if(o?.type==="base64"&&typeof o.data=="string"){const r=o.data,d=o.media_type||"image/png",f=r.startsWith("data:")?r:`data:${d};base64,${r}`;i.push({url:f})}else typeof a.url=="string"&&i.push({url:a.url})}else if(a.type==="image_url"){const o=a.image_url;typeof o?.url=="string"&&i.push({url:o.url})}}return i}function Jo(e){return l`
  ...
  `}function Yo(e,n,t,i){const s=new Date(n).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"}),a=i?.name??"Assistant";return l`
    <div class="chat-group assistant">
      ${$n("assistant",i)}
      <div class="chat-group-messages">
        ${ni({role:"assistant",content:[{type:"text",text:e}],timestamp:n},{isStreaming:!0,showReasoning:!1},t)}
  ...
  `}function Xo(e,n){const t=bn(e.role),i=n.assistantName??"Assistant",s=t==="user"?"You":t==="assistant"?i:t,a=t==="user"?"user":t==="assistant"?"assistant":"other",o=new Date(e.timestamp).toLocaleTimeString([],{hour:"numeric",minute:"2-digit"});return l`
    <div class="chat-group ${a}">
      ${$n(e.role,{name:i,avatar:n.assistantAvatar??null})}
      <div class="chat-group-messages">
        ${e.messages.map((r,d)=>ni(r.message,{isStreaming:e.isStreaming&&d===e.messages.length-1,showReasoning:n.showReasoning},n.onOpenSidebar))}
        <div class="chat-group-footer">
  ...
  `}function $n(e,n){const t=bn(e),i=n?.name?.trim()||"Assistant",s=n?.avatar?.trim()||"",a=t==="user"?"U":t==="assistant"?i.charAt(0).toUpperCase()||"A":t==="tool"?"⚙":"?",o=t==="user"?"user":t==="assistant"?"assistant":t==="tool"?"tool":"other";return s&&t==="assistant"?Zo(s)?l`<img
        class="chat-avatar ${o}"
        src="${s}"
        alt="${i}"
      />`:l`<div class="chat-avatar ${o}">${s}</div>`:l`<div class="chat-avatar ${o}">${a}</div>`}function Zo(e){return/^https?:\/\//i.test(e)||/^data:image\//i.test(e)||e.startsWith("/")}function er(e){return e.length===0?v:l`
  ...
  `}function ni(e,n,t){const i=e,s=typeof i.role=="string"?i.role:"unknown",a=Xt(e)||s.toLowerCase()==="toolresult"||s.toLowerCase()==="tool_result"||typeof i.toolCallId=="string"||typeof i.tool_call_id=="string",o=Vo(e),r=o.length>0,d=Qo(e),f=d.length>0,y=Ct(e),g=n.showReasoning&&s==="assistant"?na(e):null,c=y?.trim()?y:null,u=g?ia(g):null,b=c,$=s==="assistant"&&!!b?.trim(),w=["chat-bubble",$?"has-copy":"",n.isStreaming?"streaming":"","fade-in"].filter(Boolean).join(" ");return!b&&r&&a?l`${o.map(S=>Hn(S,t))}`:!b&&!r&&!f?v:l`
    <div class="${w}">
      ${$?Ao(b):v}
      ${er(d)}
  ...
  `;Fe([we({type:Number})],Y.prototype,"splitRatio",2);Fe([we({type:Number})],Y.prototype,"minRatio",2);Fe([we({type:Number})],Y.prototype,"maxRatio",2);Y=Fe([ot("resizable-divider")],Y);const sr=5e3;function zn(e){e.style.height="auto",e.style.height=`${e.scrollHeight}px`}function ar(e){return e?e.active?l`
      <div class="callout info compaction-indicator compaction-indicator--active">
        ${M.loader} Compacting context...
      </div>
    `:e.completedAt&&Date.now()-e.completedAt<sr?l`
        <div class="callout success compaction-indicator compaction-indicator--complete">
          ${M.check} Context compacted
        </div>
      `:v:v}function lr(){return`att-${Date.now()}-${Math.random().toString(36).slice(2,9)}`}function or(e,n){const t=e.clipboardData?.items;if(!t||!n.onAttachmentsChange)return;const i=[];for(let s=0;s<t.length;s++){const a=t[s];a.type.startsWith("image/")&&i.push(a)}if(i.length!==0){e.preventDefault();for(const s of i){const a=s.getAsFile();if(!a)continue;const o=new FileReader;o.addEventListener("load",()=>{const r=o.result,d={id:lr(),dataUrl:r,mimeType:a.type},f=n.attachments??[];n.onAttachmentsChange?.([...f,d])}),o.readAsDataURL(a)}}}function rr(e){const n=e.attachments??[];return n.length===0?v:l`
  ...
  `}function ti(e){const n=e.connected,t=e.sending||e.stream!==null,i=!!(e.canAbort&&e.onAbort),a=e.sessions?.sessions?.find(u=>u.key===e.sessionKey)?.reasoningLevel??"off",o=e.showThinking&&a!=="off",r={name:e.assistantName,avatar:e.assistantAvatar??e.assistantAvatarUrl??null},d=(e.attachments?.length??0)>0,f=e.connected?d?"Add a message or paste more images...":"Message (↩ to send, Shift+↩ for line breaks, paste images)":"Connect to the gateway to start chatting…",y=e.splitRatio??.6,g=!!(e.sidebarOpen&&e.onCloseSidebar),c=l`
    ...
    >
      ${e.loading?l`
            ...
            `:v}
      ${Bt(dr(e),u=>u.key,u=>u.kind==="reading-indicator"?Jo(r):u.kind==="stream"?Yo(u.text,u.startedAt,e.onOpenSidebar,r):u.kind==="group"?Xo(u,{onOpenSidebar:e.onOpenSidebar,showReasoning:o,assistantName:e.assistantName,assistantAvatar:r.avatar}):v)}
    </div>
  `;return l`
      ...
      >
        ...
        ${g?l`
              ...
              <div class="chat-sidebar">
                ${nr({content:e.sidebarContent??null,error:e.sidebarError??null,onClose:e.onCloseSidebar,onViewRawText:()=>{!e.sidebarContent||!e.onOpenSidebar||e.onOpenSidebar(`\`\`\`
${e.sidebarContent}
\`\`\``)}})}
              </div>
            `:v}
        ...
        <div class="chat-compose__row">
          <label class="field chat-compose__field">
            ...
            <textarea
              ...
              @input=${u=>{const b=u.target;zn(b),e.onDraftChange(b.value)}}
              @paste=${u=>or(u,e)}
              placeholder=${f}
            ></textarea>
          </label>
  ...
  `}const Vn=200;function cr(e){const n=[];let t=null;for(const i of e){if(i.kind!=="message"){t&&(n.push(t),t=null),n.push(i);continue}const s=Yt(i.message),a=bn(s.role),o=s.timestamp||Date.now();!t||t.role!==a?(t&&n.push(t),t={kind:"group",key:`group:${a}:${i.key}`,role:a,messages:[{message:i.message,key:i.key}],timestamp:o,isStreaming:!1}):t.messages.push({message:i.message,key:i.key})}return t&&n.push(t),n}function dr(e){const n=[],t=Array.isArray(e.messages)?e.messages:[],i=Array.isArray(e.toolMessages)?e.toolMessages:[],s=Math.max(0,t.length-Vn);s>0&&n.push({kind:"message",key:"chat:history:notice",message:{role:"system",content:`Showing last ${Vn} messages (${s} hidden).`,timestamp:Date.now()}});for(let a=s;a<t.length;a++){const o=t[a],r=Yt(o);!e.showThinking&&r.role.toLowerCase()==="toolresult"||n.push({kind:"message",key:qn(o,a),message:o})}if(e.showThinking)for(let a=0;a<i.length;a++)n.push({kind:"message",key:qn(i[a],a+t.length),message:i[a]});if(e.stream!==null){const a=`stream:${e.sessionKey}:${e.streamStartedAt??"live"}`;e.stream.trim().length>0?n.push({kind:"stream",key:a,text:e.stream,startedAt:e.streamStartedAt??Date.now()}):n.push({kind:"reading-indicator",key:a})}return cr(n)}function qn(e,n){const t=e,i=typeof t.toolCallId=="string"?t.toolCallId:"";if(i)return`tool:${i}`;const s=typeof t.id=="string"?t.id:"";if(s)return`msg:${s}`;const a=typeof t.messageId=="string"?t.messageId:"";if(a)return`msg:${a}`;const o=typeof t.timestamp=="number"?t.timestamp:null,r=typeof t.role=="string"?t.role:"unknown";return o!=null?`msg:${r}:${o}:${n}`:`msg:${r}:${n}`}function ur(e){const n=ti(e);return l`
  ...
  `},Gn=[{key:"env",label:"Environment"},{key:"update",label:"Updates"},{key:"agents",label:"Agents"},{key:"auth",label:"Authentication"},{key:"channels",label:"Channels"},{key:"messages",label:"Messages"},{key:"commands",label:"Commands"},{key:"hooks",label:"Hooks"},{key:"skills",label:"Skills"},{key:"tools",label:"Tools"},{key:"gateway",label:"Gateway"},{key:"wizard",label:"Setup Wizard"}],Wn="__all__";function Qn(e){return an[e]??an.default}function gr(e,n){const t=yn[e];return t||{label:n?.title??K(e),description:n?.description??""}}function vr(e){const{key:n,schema:t,uiHints:i}=e;if(!t||B(t)!=="object"||!t.properties)return[];const s=Object.entries(t.properties).map(([a,o])=>{const r=T([n,a],i),d=r?.label??o.title??K(a),f=r?.help??o.description??"",y=r?.order??50;return{key:a,label:d,description:f,order:y}});return s.sort((a,o)=>a.order!==o.order?a.order-o.order:a.key.localeCompare(o.key)),s}function fr(e,n){if(!e||!n)return[];const t=[];function i(s,a,o){if(s===a)return;if(typeof s!=typeof a){t.push({path:o,from:s,to:a});return}if(typeof s!="object"||s===null||a===null){s!==a&&t.push({path:o,from:s,to:a});return}if(Array.isArray(s)&&Array.isArray(a)){JSON.stringify(s)!==JSON.stringify(a)&&t.push({path:o,from:s,to:a});return}const r=s,d=a,f=new Set([...Object.keys(r),...Object.keys(d)]);for(const y of f)i(r[y],d[y],o?`${o}.${y}`:y)}return i(e,n,""),t}function Jn(e,n=40){let t;try{t=JSON.stringify(e)??String(e)}catch{t=String(e)}return t.length<=n?t:t.slice(0,n-3)+"..."}function pr(e){const n=e.valid==null?"unknown":e.valid?"valid":"invalid",t=qt(e.schema),i=t.schema?t.unsupportedPaths.length>0:!1,s=t.schema?.properties??{},a=Gn.filter(k=>k.key in s),o=new Set(Gn.map(k=>k.key)),r=Object.keys(s).filter(k=>!o.has(k)).map(k=>({key:k,label:k.charAt(0).toUpperCase()+k.slice(1)})),d=[...a,...r],f=e.activeSection&&t.schema&&B(t.schema)==="object"?t.schema.properties?.[e.activeSection]:void 0,y=e.activeSection?gr(e.activeSection,f):null,g=e.activeSection?vr({key:e.activeSection,schema:f,uiHints:e.uiHints}):[],c=e.formMode==="form"&&!!e.activeSection&&g.length>0,u=e.activeSubsection===Wn,b=e.searchQuery||u?null:e.activeSubsection??g[0]?.key??null,$=e.formMode==="form"?fr(e.originalValue,e.formValue):[],w=e.formMode==="raw"&&e.raw!==e.originalRaw,S=e.formMode==="form"?$.length>0:w,x=!!e.formValue&&!e.loading&&!!t.schema,A=e.connected&&!e.saving&&S&&(e.formMode==="raw"?!0:x),L=e.connected&&!e.applying&&!e.updating&&S&&(e.formMode==="raw"?!0:x),C=e.connected&&!e.applying&&!e.updating;return l`
        ...
        <div class="config-content">
          ${e.formMode==="form"?l`
                ${e.schemaLoading?l`
                      ...
                      `:Bl({schema:t.schema,uiHints:e.uiHints,value:e.formValue,disabled:e.loading||!e.formValue,unsupportedPaths:t.unsupportedPaths,onPatch:e.onFormPatch,searchQuery:e.searchQuery,activeSection:e.activeSection,activeSubsection:b})}
                ${i?l`
...
${JSON.stringify(e.issues,null,2)}</pre
  ...
  `}function hr(e){const n=["last",...e.channels.filter(Boolean)],t=e.form.channel?.trim();t&&!n.includes(t)&&n.push(t);const i=new Set;return n.filter(s=>i.has(s)?!1:(i.add(s),!0))}function mr(e,n){if(n==="last")return"last";const t=e.channelMeta?.find(i=>i.id===n);return t?.label?t.label:e.channelLabels?.[n]??n}function yr(e){const n=hr(e);return l`
    <section class="grid grid-cols-2">
      <div class="card">
        <div class="card-title">Scheduler</div>
  ...
  `}function br(e){const n=e.form;return n.scheduleKind==="at"?l`
      <label class="field" style="margin-top: 12px;">
        <span>Run at</span>
        <input
          type="datetime-local"
  ...
  `}function $r(e,n){const i=`list-item list-item-clickable${n.runsJobId===e.id?" list-item-selected":""}`;return l`
    <div class=${i} @click=${()=>n.onLoadRuns(e.id)}>
      <div class="list-main">
        <div class="list-title">${e.name}</div>
        <div class="list-sub">${Kt(e)}</div>
        <div class="muted">${Ot(e)}</div>
  ...
  `}function Sr(e){const t=(e.status&&typeof e.status=="object"?e.status.securityAudit:null)?.summary??null,i=t?.critical??0,s=t?.warn??0,a=t?.info??0,o=i>0?"danger":s>0?"warn":"success",r=i>0?`${i} critical`:s>0?`${s} warnings`:"No critical issues";return l`
    <section class="grid grid-cols-2">
      <div class="card">
        <div class="row" style="justify-content: space-between;">
          <div>
            ...
            <div class="list" style="margin-top: 12px;">
              ${e.eventLog.map(d=>l`
                  <div class="list-item">
                    ...
                    <div class="list-meta">
                      <pre class="code-block">${Za(d.payload)}</pre>
                    </div>
                  </div>
                `)}
            </div>
  ...
  `}function xr(e){const n=Math.max(0,e),t=Math.floor(n/1e3);if(t<60)return`${t}s`;const i=Math.floor(t/60);return i<60?`${i}m`:`${Math.floor(i/60)}h`}function q(e,n){return n?l`<div class="exec-approval-meta-row"><span>${e}</span><span>${n}</span></div>`:v}function kr(e){const n=e.execApprovalQueue[0];if(!n)return v;const t=n.request,i=n.expiresAtMs-Date.now(),s=i>0?`expires in ${xr(i)}`:"expired",a=e.execApprovalQueue.length;return l`
  ...
  `}function Ar(e){const{pendingGatewayUrl:n}=e;return n?l`
    ...
    <section class="card">
      ...
      <div class="list" style="margin-top: 16px;">
        ${e.entries.length===0?l`
              ...
              `:e.entries.map(n=>Lr(n))}
      </div>
    </section>
  `}function Lr(e){const n=e.lastInputSeconds!=null?`${e.lastInputSeconds}s ago`:"n/a",t=e.mode??"unknown",i=Array.isArray(e.roles)?e.roles.filter(Boolean):[],s=Array.isArray(e.scopes)?e.scopes.filter(Boolean):[],a=s.length>0?s.length>3?`${s.length} scopes`:`scopes: ${s.join(", ")}`:null;return l`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${e.host??"unknown host"}</div>
        <div class="list-sub">${Ja(e)}</div>
        <div class="chip-row">
  ...
  `}const Yn=["trace","debug","info","warn","error","fatal"];function Ir(e){if(!e)return"";const n=new Date(e);return Number.isNaN(n.getTime())?e:n.toLocaleTimeString()}function _r(e,n){return n?[e.message,e.subsystem,e.raw].filter(Boolean).join(" ").toLowerCase().includes(n):!0}function Mr(e){const n=e.filterText.trim().toLowerCase(),t=Yn.some(a=>!e.levelFilters[a]),i=e.entries.filter(a=>a.level&&!e.levelFilters[a.level]?!1:_r(a,n)),s=n||t?"filtered":"visible";return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
      ...
      <div class="log-stream" style="margin-top: 12px;" @scroll=${e.onScroll}>
        ${i.length===0?l`
              ...
              `:i.map(a=>l`
                <div class="log-row">
                  <div class="log-time mono">${Ir(a.time)}</div>
  ...
  `}function Er(e){const n=Nr(e),t=Hr(e);return l`
    ${Vr(t)}
    ${zr(n)}
    ${Fr(e)}
    <section class="card">
  ...
  `}function Fr(e){const n=e.devicesList??{pending:[],paired:[]},t=Array.isArray(n.pending)?n.pending:[],i=Array.isArray(n.paired)?n.paired:[];return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
  ...
  `}function Tr(e,n){const t=e.displayName?.trim()||e.deviceId,i=typeof e.ts=="number"?_(e.ts):"n/a",s=e.role?.trim()?`role: ${e.role}`:"role: -",a=e.isRepair?" · repair":"",o=e.remoteIp?` · ${e.remoteIp}`:"";return l`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function Rr(e,n){const t=e.displayName?.trim()||e.deviceId,i=e.remoteIp?` · ${e.remoteIp}`:"",s=`roles: ${We(e.roles)}`,a=`scopes: ${We(e.scopes)}`,o=Array.isArray(e.tokens)?e.tokens:[];return l`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${t}</div>
        ...
        ${o.length===0?l`
              ...
              <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 6px;">
                ${o.map(r=>Pr(e.deviceId,r,n))}
              </div>
            `}
      </div>
    </div>
  `}function Pr(e,n,t){const i=n.revokedAtMs?"revoked":"active",s=`scopes: ${We(n.scopes)}`,a=_(n.rotatedAtMs??n.createdAtMs??n.lastUsedAtMs??null);return l`
  ...
  `}const j="__defaults__",Xn=[{value:"deny",label:"Deny"},{value:"allowlist",label:"Allowlist"},{value:"full",label:"Full"}],Br=[{value:"off",label:"Off"},{value:"on-miss",label:"On miss"},{value:"always",label:"Always"}];function Nr(e){const n=e.configForm,t=Xr(e.nodes),{defaultBinding:i,agents:s}=ec(n),a=!!n,o=e.configSaving||e.configFormMode==="raw";return{ready:a,disabled:o,configDirty:e.configDirty,configLoading:e.configLoading,configSaving:e.configSaving,defaultBinding:i,agents:s,nodes:t,onBindDefault:e.onBindDefault,onBindAgent:e.onBindAgent,onSave:e.onSaveBindings,onLoadConfig:e.onLoadConfig,formMode:e.configFormMode}}function Zn(e){return e==="allowlist"||e==="full"||e==="deny"?e:"deny"}function Dr(e){return e==="always"||e==="off"||e==="on-miss"?e:"on-miss"}function Kr(e){const n=e?.defaults??{};return{security:Zn(n.security),ask:Dr(n.ask),askFallback:Zn(n.askFallback??"deny"),autoAllowSkills:!!(n.autoAllowSkills??!1)}}function Or(e){const n=e?.agents??{},t=Array.isArray(n.list)?n.list:[],i=[];return t.forEach(s=>{if(!s||typeof s!="object")return;const a=s,o=typeof a.id=="string"?a.id.trim():"";if(!o)return;const r=typeof a.name=="string"?a.name.trim():void 0,d=a.default===!0;i.push({id:o,name:r||void 0,isDefault:d})}),i}function Ur(e,n){const t=Or(e),i=Object.keys(n?.agents??{}),s=new Map;t.forEach(o=>s.set(o.id,o)),i.forEach(o=>{s.has(o)||s.set(o,{id:o})});const a=Array.from(s.values());return a.length===0&&a.push({id:"main",isDefault:!0}),a.sort((o,r)=>{if(o.isDefault&&!r.isDefault)return-1;if(!o.isDefault&&r.isDefault)return 1;const d=o.name?.trim()?o.name:o.id,f=r.name?.trim()?r.name:r.id;return d.localeCompare(f)}),a}function jr(e,n){return e===j?j:e&&n.some(t=>t.id===e)?e:j}function Hr(e){const n=e.execApprovalsForm??e.execApprovalsSnapshot?.file??null,t=!!n,i=Kr(n),s=Ur(e.configForm,n),a=Zr(e.nodes),o=e.execApprovalsTarget;let r=o==="node"&&e.execApprovalsTargetNodeId?e.execApprovalsTargetNodeId:null;o==="node"&&r&&!a.some(g=>g.id===r)&&(r=null);const d=jr(e.execApprovalsSelectedAgent,s),f=d!==j?(n?.agents??{})[d]??null:null,y=Array.isArray(f?.allowlist)?f.allowlist??[]:[];return{ready:t,disabled:e.execApprovalsSaving||e.execApprovalsLoading,dirty:e.execApprovalsDirty,loading:e.execApprovalsLoading,saving:e.execApprovalsSaving,form:n,defaults:i,selectedScope:d,selectedAgent:f,agents:s,allowlist:y,target:o,targetNodeId:r,targetNodes:a,onSelectScope:e.onExecApprovalsSelectAgent,onSelectTarget:e.onExecApprovalsTargetChange,onPatch:e.onExecApprovalsPatch,onRemove:e.onExecApprovalsRemove,onLoad:e.onLoadExecApprovals,onSave:e.onSaveExecApprovals}}function zr(e){const n=e.nodes.length>0,t=e.defaultBinding??"";return l`
    <section class="card">
  ...
  `}function Vr(e){const n=e.ready,t=e.target!=="node"||!!e.targetNodeId;return l`
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        <div>
  ...
  `}function qr(e){const n=e.targetNodes.length>0,t=e.targetNodeId??"";return l`
    <div class="list" style="margin-top: 12px;">
      <div class="list-item">
        <div class="list-main">
  ...
  `}function Qr(e){const n=["agents",e.selectedScope,"allowlist"],t=e.allowlist;return l`
  ...
  `}function Jr(e,n,t){const i=n.lastUsedAt?_(n.lastUsedAt):"never",s=n.lastUsedCommand?Qe(n.lastUsedCommand,120):null,a=n.lastResolvedPath?Qe(n.lastResolvedPath,120):null;return l`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function Yr(e,n){const t=e.binding??"__default__",i=e.name?.trim()?`${e.name} (${e.id})`:e.id,s=n.nodes.length>0;return l`
    <div class="list-item">
      <div class="list-main">
  ...
  `}function Xr(e){const n=[];for(const t of e){if(!(Array.isArray(t.commands)?t.commands:[]).some(r=>String(r)==="system.run"))continue;const a=typeof t.nodeId=="string"?t.nodeId.trim():"";if(!a)continue;const o=typeof t.displayName=="string"&&t.displayName.trim()?t.displayName.trim():a;n.push({id:a,label:o===a?a:`${o} · ${a}`})}return n.sort((t,i)=>t.label.localeCompare(i.label)),n}function Zr(e){const n=[];for(const t of e){if(!(Array.isArray(t.commands)?t.commands:[]).some(r=>String(r)==="system.execApprovals.get"||String(r)==="system.execApprovals.set"))continue;const a=typeof t.nodeId=="string"?t.nodeId.trim():"";if(!a)continue;const o=typeof t.displayName=="string"&&t.displayName.trim()?t.displayName.trim():a;n.push({id:a,label:o===a?a:`${o} · ${a}`})}return n.sort((t,i)=>t.label.localeCompare(i.label)),n}function ec(e){const n={id:"main",name:void 0,index:0,isDefault:!0,binding:null};if(!e||typeof e!="object")return{defaultBinding:null,agents:[n]};const i=(e.tools??{}).exec??{},s=typeof i.node=="string"&&i.node.trim()?i.node.trim():null,a=e.agents??{},o=Array.isArray(a.list)?a.list:[];if(o.length===0)return{defaultBinding:s,agents:[n]};const r=[];return o.forEach((d,f)=>{if(!d||typeof d!="object")return;const y=d,g=typeof y.id=="string"?y.id.trim():"";if(!g)return;const c=typeof y.name=="string"?y.name.trim():void 0,u=y.default===!0,$=(y.tools??{}).exec??{},w=typeof $.node=="string"&&$.node.trim()?$.node.trim():null;r.push({id:g,name:c||void 0,index:f,isDefault:u,binding:w})}),r.length===0&&r.push(n),{defaultBinding:s,agents:r}}function nc(e){const n=!!e.connected,t=!!e.paired,i=typeof e.displayName=="string"&&e.displayName.trim()||(typeof e.nodeId=="string"?e.nodeId:"unknown"),s=Array.isArray(e.caps)?e.caps:[],a=Array.isArray(e.commands)?e.commands:[];return l`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">${i}</div>
        <div class="list-sub">
  ...
  `}function tc(e){const n=e.hello?.snapshot,t=n?.uptimeMs?at(n.uptimeMs):"n/a",i=n?.policy?.tickIntervalMs?`${n.policy.tickIntervalMs}ms`:"n/a",s=(()=>{if(e.connected||!e.lastError)return null;const o=e.lastError.toLowerCase();if(!(o.includes("unauthorized")||o.includes("connect failed")))return null;const d=!!e.settings.token.trim(),f=!!e.password.trim();return!d&&!f?l`
        <div class="muted" style="margin-top: 8px">
      ...
      <div class="card">
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          ...
          <div class="stat">
            ...
            <div class="stat-value">
              ${e.lastChannelsRefresh?_(e.lastChannelsRefresh):"n/a"}
            </div>
          </div>
        </div>
  ...
  `}const ic=["","off","minimal","low","medium","high"],sc=["","off","on"],ac=[{value:"",label:"inherit"},{value:"off",label:"off (explicit)"},{value:"on",label:"on"}],lc=["","off","on","stream"];function oc(e){if(!e)return"";const n=e.trim().toLowerCase();return n==="z.ai"||n==="z-ai"?"zai":n}function ii(e){return oc(e)==="zai"}function rc(e){return ii(e)?sc:ic}function cc(e,n){return!n||!e||e==="off"?e:"on"}function dc(e,n){return e?n&&e==="on"?"low":e:null}function uc(e){const n=e.result?.sessions??[];return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
      ...
      <div class="table" style="margin-top: 16px;">
        ...
        ${n.length===0?l`
              ...
              `:n.map(t=>gc(t,e.basePath,e.onPatch,e.onDelete,e.loading))}
      </div>
    </section>
  `}function gc(e,n,t,i,s){const a=e.updatedAt?_(e.updatedAt):"n/a",o=e.thinkingLevel??"",r=ii(e.modelProvider),d=cc(o,r),f=rc(e.modelProvider),y=e.verboseLevel??"",g=e.reasoningLevel??"",c=e.displayName??e.key,u=e.kind!=="global",b=u?`${fn("chat",n)}?session=${encodeURIComponent(e.key)}`:null;return l`
    <div class="table-row">
      ...
      <div>
        <select
          .value=${d}
          ?disabled=${s}
          @change=${$=>{const w=$.target.value;t(e.key,{thinkingLevel:dc(w,r)})}}
        >
      ...
      <div>
        ...
        >
          ${ac.map($=>l`<option value=${$.value}>${$.label}</option>`)}
        </select>
      </div>
      <div>
        <select
        ...
        >
          ${lc.map($=>l`<option value=${$}>${$||"inherit"}</option>`)}
        </select>
      </div>
      <div>
  ...
  `}const ze=[{id:"workspace",label:"Workspace Skills",sources:["openclaw-workspace"]},{id:"built-in",label:"Built-in Skills",sources:["openclaw-bundled"]},{id:"installed",label:"Installed Skills",sources:["openclaw-managed"]},{id:"extra",label:"Extra Skills",sources:["openclaw-extra"]}];function vc(e){const n=new Map;for(const s of ze)n.set(s.id,{id:s.id,label:s.label,skills:[]});const t={id:"other",label:"Other Skills",skills:[]};for(const s of e){const a=ze.find(o=>o.sources.includes(s.source));a?n.get(a.id)?.skills.push(s):t.skills.push(s)}const i=ze.map(s=>n.get(s.id)).filter(s=>!!(s&&s.skills.length>0));return t.skills.length>0&&i.push(t),i}function fc(e){const n=e.report?.skills??[],t=e.filter.trim().toLowerCase(),i=t?n.filter(a=>[a.name,a.description,a.source].join(" ").toLowerCase().includes(t)):n,s=vc(i);return l`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">Skills</div>
  ...
  `}function pc(e,n){const t=n.busyKey===e.skillKey,i=n.edits[e.skillKey]??"",s=n.messages[e.skillKey]??null,a=e.install.length>0&&e.missing.bins.length>0,o=[...e.missing.bins.map(d=>`bin:${d}`),...e.missing.env.map(d=>`env:${d}`),...e.missing.config.map(d=>`config:${d}`),...e.missing.os.map(d=>`os:${d}`)],r=[];return e.disabled&&r.push("disabled"),e.blockedByAllowlist&&r.push("blocked by allowlist"),l`
    <div class="list-item">
      <div class="list-main">
        <div class="list-title">
          ${e.emoji?`${e.emoji} `:""}${e.name}
  ...
  `}const hc=/^data:/i,mc=/^https?:\/\//i;function yc(e){const n=e.agentsList?.agents??[],i=vt(e.sessionKey)?.agentId??e.agentsList?.defaultId??"main",a=n.find(r=>r.id===i)?.identity,o=a?.avatarUrl??a?.avatar;if(o)return hc.test(o)||mc.test(o)?o:a?.avatarUrl}function bc(e){const n=e.presenceEntries.length,t=e.sessionsResult?.count??null,i=e.cronStatus?.nextWakeAtMs??null,s=e.connected?null:"Disconnected from gateway.",a=e.tab==="chat"||e.tab==="taxchat",o=a&&(e.settings.chatFocusMode||e.onboarding),r=e.onboarding?!1:e.settings.chatShowThinking,d=yc(e),f=e.chatAvatarUrl??d??null,y=e.configForm??e.configSnapshot?.config,g=e.agentsSelectedId??e.agentsList?.defaultId??e.agentsList?.agents?.[0]?.id??null;return l`
        ...
        <div class="topbar-left">
          ...
          <div class="brand">
            <div class="brand-logo">
              <img src="/favicon-32.png" alt="Taxbot" />
            </div>
            <div class="brand-text">
              <div class="brand-title">Taxbot</div>
