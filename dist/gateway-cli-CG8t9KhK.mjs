import { B as normalizeThinkLevel, F as formatXHighModelHint, H as normalizeVerboseLevel, P as formatThinkingLevels, R as normalizeElevatedLevel, V as normalizeUsageDisplay, W as supportsXHighThinking, z as normalizeReasoningLevel } from "./pi-embedded-helpers-DIV_C5J8.mjs";
import { $ as migrateLegacyCronPayload, $r as setTtsProvider, $t as getRemoteSkillEligibility, Ai as DEFAULT_INPUT_PDF_MAX_PIXELS, Bn as onHeartbeatEvent, Br as registerInternalHook, C as resolveSessionModelRef, Ci as DEFAULT_INPUT_FILE_MAX_BYTES, D as readSessionPreviewItemsFromTranscript, Di as DEFAULT_INPUT_IMAGE_MIMES, E as readSessionMessages, Ei as DEFAULT_INPUT_IMAGE_MAX_BYTES, En as normalizeGroupActivation, F as resolveOutboundTarget, Fi as normalizeMimeList, Fr as formatUserTime, Gr as isTtsEnabled, Gt as CONTROL_UI_AVATAR_PREFIX, Hr as OPENAI_TTS_MODELS, I as resolveSessionDeliveryTarget, Ir as resolveUserTimeFormat, J as isGatewaySigusr1RestartExternallyAllowed, Jn as getChannelActivity, Jr as resolveTtsAutoMode, Jt as resolveAssistantAvatarUrl, K as authorizeGatewaySigusr1Restart, Kr as isTtsProviderConfigured, Kt as buildControlUiAvatarUrl, Ln as resolveHeartbeatVisibility, Lr as resolveUserTimezone, M as resolveOutboundSessionRoute, Mi as DEFAULT_INPUT_TIMEOUT_MS, Mr as resolveEffectiveMessagesConfig, Ni as extractFileContentFromSource, Nr as resolveIdentityName, O as resolveSessionTranscriptCandidates, Oi as DEFAULT_INPUT_MAX_REDIRECTS, On as registerUnhandledRejectionHandler, Pi as extractImageContentFromSource, Pt as handleReset, Q as normalizeCronJobPatch, Qr as setTtsEnabled, R as resetDirectoryCache, Rr as clearInternalHooks, S as resolveGatewaySessionStoreTarget, Si as loadModelCatalog, T as capArrayByJsonBytes, Ti as DEFAULT_INPUT_FILE_MIMES, Ui as resolveAgentTimeoutMs, Ur as OPENAI_TTS_VOICES, Vr as triggerInternalHook, W as runWithModelFallback, Wr as getTtsProvider, X as setGatewaySigusr1RestartPolicy, Xr as resolveTtsPrefsPath, Y as scheduleGatewaySigusr1Restart, Yr as resolveTtsConfig, Z as normalizeCronJobCreate, Zr as resolveTtsProviderOrder, _ as resolveAnnounceTargetFromKey, _i as applyVerboseOverride, _r as getAgentRunContext, a as loadCostUsageSummary, ai as stopDiagnosticHeartbeat, an as approveNodePairing, b as loadCombinedSessionStoreForGateway, bi as isSystemEventContextChanged, c as clearSessionQueues, cn as renamePairedNode, ct as loadProviderUsageSummary, d as waitForEmbeddedPiRunEnd, dn as verifyNodeToken, dr as isAbortTrigger, ei as textToSpeech, en as primeRemoteSkillsCache, f as createOpenClawTools, fn as getSkillsSnapshotVersion, fr as stopSubagentsForRequester, g as isExternalHookSession, gi as applyModelOverrideToSessionEntry, gr as emitAgentEvent, gt as handleSlackHttpRequest, h as getHookType, hr as clearAgentRunContext, i as runCliAgent, ii as startDiagnosticHeartbeat, in as setSkillsRemoteRegistry, ir as extractShortModelName, it as requestHeartbeatNow, j as ensureOutboundSessionEntry, ji as DEFAULT_INPUT_PDF_MIN_TEXT_CHARS, jn as buildHistoryContextFromEntries, jr as resolveAgentIdentity, k as stripEnvelopeFromMessages, ki as DEFAULT_INPUT_PDF_MAX_PAGES, l as runEmbeddedPiAgent, li as DEFAULT_HEARTBEAT_ACK_MAX_CHARS, ln as requestNodePairing, m as detectSuspiciousPatterns, mi as lookupContextTokens, n as getCliSessionId, ni as setCommandLaneConcurrency, nn as refreshRemoteBinsForConnectedNodes, nr as dispatchInboundMessage, o as normalizeSendPolicy, on as listNodePairing, ot as getPluginToolMeta, p as buildSafeExternalPrompt, pi as stripHeartbeatToken, pn as registerSkillsChangeListener, pr as initSubagentRegistry, pt as hasNonzeroUsage, q as consumeGatewaySigusr1RestartAuthorization, qr as resolveTtsApiKey, qt as normalizeControlUiBasePath, r as setCliSessionId, ri as CommandLane, rn as refreshRemoteNodeBins, rr as createReplyDispatcher, s as resolveSendPolicy, si as isDiagnosticsEnabled, sn as rejectNodePairing, st as loadOpenClawPlugins, tn as recordRemoteNodeInfo, u as abortEmbeddedPiRun, un as updatePairedNodeMetadata, ur as formatZonedTimestamp, v as listAgentsForGateway, vi as parseVerboseOverride, vr as onAgentEvent, w as archiveFileOnDisk, wi as DEFAULT_INPUT_FILE_MAX_CHARS, x as loadSessionEntry, xr as normalizePollInput, y as listSessionsFromStore, yi as enqueueSystemEvent, yr as registerAgentRunContext, zn as getLastHeartbeatEvent, zr as createInternalHookEvent } from "./reply-CReGWUt7.mjs";
import { g as resolveStateDir, i as isNixMode, l as resolveGatewayLockDir, o as resolveConfigPath, r as STATE_DIR, t as CONFIG_PATH, u as resolveGatewayPort } from "./paths-DcBT4Q4r.mjs";
import { A as buildAgentMainSessionKey, D as DEFAULT_ACCOUNT_ID, E as resolveOpenClawPackageRoot, F as normalizeMainKey, H as isSubagentSessionKey, I as resolveAgentIdFromSessionKey, P as normalizeAgentId, T as resolveWorkspaceTemplateDir, U as parseAgentSessionKey, _ as DEFAULT_MEMORY_FILENAME, b as DEFAULT_USER_FILENAME, c as resolveDefaultAgentId, d as DEFAULT_AGENTS_FILENAME, g as DEFAULT_MEMORY_ALT_FILENAME, h as DEFAULT_IDENTITY_FILENAME, i as resolveAgentModelFallbacksOverride, l as resolveSessionAgentId, m as DEFAULT_HEARTBEAT_FILENAME, n as resolveAgentConfig, p as DEFAULT_BOOTSTRAP_FILENAME, r as resolveAgentDir, s as resolveAgentWorkspaceDir, t as listAgentIds, v as DEFAULT_SOUL_FILENAME, w as resolveDefaultAgentWorkspaceDir, x as ensureAgentWorkspace, y as DEFAULT_TOOLS_FILENAME, z as toAgentRequestSessionKey } from "./agent-scope-BD88ooNs.mjs";
import { B as getLogger, C as getActivePluginRegistry, I as colorize, L as isRich, M as setVerbose, R as theme, V as getResolvedLoggerSettings, a as setConsoleTimestampPrefix, c as defaultRuntime, h as DEFAULT_CHAT_CHANNEL, i as setConsoleSubsystemFilter, n as runtimeForLogger, p as CHANNEL_IDS, t as createSubsystemLogger, z as getChildLogger } from "./subsystem-iSHcgawR.mjs";
import { _ as shortenHomePath, h as resolveUserPath, s as ensureDir, t as CONFIG_DIR, x as truncateUtf16Safe } from "./utils-D9-prTJL.mjs";
import { a as logDebug, c as logWarn, n as runExec, t as runCommandWithTimeout } from "./exec-BRaJtAm2.mjs";
import { c as resolveAllowedModelRef, d as resolveHooksGmailModel, ht as DEFAULT_PROVIDER, i as isCliProvider, l as resolveConfiguredModelRef, mt as DEFAULT_MODEL, p as resolveThinkingDefault, pt as DEFAULT_CONTEXT_TOKENS, r as getModelRefStatus } from "./model-selection-DNDKru6x.mjs";
...
import { n as logAcceptedEnvOption, t as isTruthyEnvValue } from "./env-DE0tkFHW.mjs";
import { H as applyLegacyMigrations, J as VERSION, K as resolveAgentMaxConcurrent, a as parseConfigJson5, c as writeConfigFile, i as loadConfig, l as validateConfigObjectWithPlugins, n as migrateLegacyConfig, o as readConfigFileSnapshot, q as resolveSubagentMaxConcurrent, r as createConfigIO, s as resolveConfigSnapshotHash, u as OpenClawSchema } from "./config-B_uz17RB.mjs";
import { o as isTestDefaultMemorySlotDisabled } from "./manifest-registry-BADIO_87.mjs";
import { n as listChannelPlugins, r as normalizeChannelId, t as getChannelPlugin } from "./plugins-DSII6JvY.mjs";
import { B as snapshotSessionOrigin, Et as stripPluginOnlyAllowlist, F as resolveAgentMainSessionKey, I as resolveExplicitAgentSessionKey, L as resolveMainSessionKey, R as resolveMainSessionKeyFromConfig, S as mergeDeliveryContext, St as expandPolicyWithPluginGroups, Tt as resolveToolProfilePolicy, b as deliveryContextFromSession, bt as buildPluginToolGroups, d as loadSessionStore, g as updateSessionStore, w as normalizeSessionDeliveryFields, wt as normalizeToolName, xt as collectExplicitAllowlist } from "./sandbox-oRz8OYlY.mjs";
import "./image-Dsy38YsT.mjs";
import "./pi-model-discovery-B6X6meeT.mjs";
import { T as formatErrorMessage, d as inspectPortUsage, l as ensurePortAvailable, p as formatPortDiagnostics } from "./chrome-PKn2zNTL.mjs";
import { i as loadWorkspaceSkillEntries, r as buildWorkspaceSkillSnapshot, u as hasBinary } from "./skills-Cb4fWMT0.mjs";
import { l as saveMediaBuffer, u as detectMime } from "./routes-2qU-Bliw.mjs";
import "./server-context-CAvvAxGZ.mjs";
import { t as rawDataToString } from "./ws-BZde2LM5.mjs";
import { f as GATEWAY_CLIENT_IDS, i as isGatewayMessageChannel, l as normalizeMessageChannel, m as GATEWAY_CLIENT_NAMES, n as isDeliverableMessageChannel, p as GATEWAY_CLIENT_MODES, r as isGatewayCliClient, s as isWebchatClient, t as INTERNAL_MESSAGE_CHANNEL } from "./message-channel-Ls7G_LaM.mjs";
import "./logging-sIVJ3oF8.mjs";
import "./accounts-Da593ZbL.mjs";
import { o as resolveStorePath, r as resolveSessionTranscriptPath } from "./paths-D0n0xdZb.mjs";
import "./redact-DjVwGPIJ.mjs";
import "./tool-display-C07JRsU3.mjs";
import { o as normalizeReplyPayloadsForDelivery, t as deliverOutboundPayloads } from "./deliver-DeFlE7gj.mjs";
import { i as getMachineDisplayName, t as createBrowserRouteDispatcher } from "./dispatcher-CWBZae0H.mjs";
import "./manager-s7p2EtXX.mjs";
import "./sqlite-CG_dxLw3.mjs";
import "./warnings-DCJmPZde.mjs";
import { m as formatUsd, p as formatTokenCount } from "./channel-summary-CADwe6FA.mjs";
import { $ as validatePollParams, A as validateDevicePairListParams, B as validateLogsTailParams, C as validateCronListParams, Ct as ErrorCodes, D as validateCronStatusParams, Dt as buildDeviceAuthPayload, E as validateCronRunsParams, Et as parseSessionLabel, F as validateExecApprovalResolveParams, G as validateNodeInvokeResultParams, H as validateNodeDescribeParams, I as validateExecApprovalsGetParams, J as validateNodePairListParams, K as validateNodeListParams, L as validateExecApprovalsNodeGetParams, M as validateDeviceTokenRevokeParams, Mt as verifyDeviceSignature, N as validateDeviceTokenRotateParams, O as validateCronUpdateParams, P as validateExecApprovalRequestParams, Q as validateNodeRenameParams, R as validateExecApprovalsNodeSetParams, S as validateCronAddParams, St as PROTOCOL_VERSION, T as validateCronRunParams, U as validateNodeEventParams, V as validateModelsListParams, W as validateNodeInvokeParams, X as validateNodePairRequestParams, Y as validateNodePairRejectParams, Z as validateNodePairVerifyParams, _ as validateConfigGetParams, _t as validateWebLoginWaitParams, a as validateAgentWaitParams, at as validateSessionsPatchParams, b as validateConfigSetParams, bt as validateWizardStartParams, c as validateAgentsFilesSetParams, ct as validateSessionsResolveParams, d as validateChannelsStatusParams, dt as validateSkillsStatusParams, et as validateRequestFrame, f as validateChatAbortParams, ft as validateSkillsUpdateParams, g as validateConfigApplyParams, gt as validateWebLoginStartParams, h as validateChatSendParams, ht as validateWakeParams, i as validateAgentParams, it as validateSessionsListParams, j as validateDevicePairRejectParams, jt as normalizeDevicePublicKeyBase64Url, k as validateDevicePairApproveParams, kt as deriveDeviceIdFromPublicKey, l as validateAgentsListParams, lt as validateSkillsBinsParams, m as validateChatInjectParams, mt as validateUpdateRunParams, n as formatValidationErrors, nt as validateSessionsCompactParams, o as validateAgentsFilesGetParams, ot as validateSessionsPreviewParams, p as validateChatHistoryParams, pt as validateTalkModeParams, q as validateNodePairApproveParams, r as validateAgentIdentityParams, rt as validateSessionsDeleteParams, s as validateAgentsFilesListParams, st as validateSessionsResetParams, tt as validateSendParams, u as validateChannelsLogoutParams, ut as validateSkillsInstallParams, v as validateConfigPatchParams, vt as validateWizardCancelParams, w as validateCronRemoveParams, wt as errorShape, x as validateConnectParams, xt as validateWizardStatusParams, y as validateConfigSchemaParams, yt as validateWizardNextParams, z as validateExecApprovalsSetParams } from "./client-DgdwkhIS.mjs";
import { n as pickPrimaryTailnetIPv6, t as pickPrimaryTailnetIPv4 } from "./tailnet-CobctoGg.mjs";
import { i as loadGatewayTlsRuntime$1, n as callGateway } from "./call-DrwxBANw.mjs";
import "./login-qr-BEDNHQPL.mjs";
import "./pairing-store-bEGBKxWn.mjs";
import { t as formatDocsLink } from "./links-DX7cRSHU.mjs";
import { r as runCommandWithRuntime } from "./cli-utils-d8bOKFO2.mjs";
import { n as withProgress } from "./progress-BYrleoMX.mjs";
import { a as resolveSubagentToolPolicy, i as resolveGroupToolPolicy, r as resolveEffectiveToolPolicy, t as filterToolsByPolicy } from "./pi-tools.policy-DvvMN9Ge.mjs";
import "./prompt-style-DXmBJpDK.mjs";
import "./pairing-labels-JtCdT618.mjs";
import { c as normalizeExecApprovals, g as saveExecApprovals, l as readExecApprovalsSnapshot, m as resolveExecApprovalsSocketPath, r as ensureExecApprovals } from "./exec-approvals-hhzj9sO5.mjs";
import { n as createBrowserControlContext, r as startBrowserControlServiceFromConfig } from "./control-service-tDwWDpUV.mjs";
import { a as summarizeRestartSentinel, n as formatDoctorNonInteractiveHint, r as formatRestartSentinelMessage, s as writeRestartSentinel, t as consumeRestartSentinel } from "./restart-sentinel-BSJCs5vQ.mjs";
import { n as resolveMessageChannelSelection } from "./channel-selection-CFHdlQsS.mjs";
import { n as createOutboundSendDeps, t as createDefaultDeps } from "./deps-C1EbislO.mjs";
import { i as enableTailscaleServe, n as disableTailscaleServe, o as getTailnetHostname, r as enableTailscaleFunnel, t as disableTailscaleFunnel } from "./tailscale-4Z5W8e4e.mjs";
import { t as ensureOpenClawCliOnPath } from "./path-env-AOlAvuGZ.mjs";
import { t as forceFreePortAndWait } from "./ports-Co7xga5_.mjs";
import { n as resolveChannelDefaultAccountId } from "./helpers--sAxmx1H.mjs";
import "./logging-BTIZSnJm.mjs";
import { t as WizardCancelledError } from "./prompts-TEa_gmaq.mjs";
import { c as loadAgentIdentity, d as loadAgentIdentityFromWorkspace, r as getStatusSummary, t as runOnboardingWizard } from "./onboarding-81PdWYa6.mjs";
import { i as discoverGatewayBeacons, n as installSkill } from "./onboard-skills-CJmeZwSj.mjs";
import "./github-copilot-auth-DLp_veWW.mjs";
import "./onboard-channels-B7Nzchqn.mjs";
import { r as buildChannelUiCatalog, t as applyPluginAutoEnable } from "./plugin-auto-enable-DtZr53tO.mjs";
import "./archive-7jXYzIVl.mjs";
import "./installs-ChQ-kMei.mjs";
import { c as startHeartbeatRunner, n as formatHealthChannelLines, o as runHeartbeatOnce, r as getHealthSnapshot, s as setHeartbeatsEnabled } from "./health-format-CQglK8Ez.mjs";
import { c as compareSemverStrings, f as DEFAULT_PACKAGE_CHANNEL, h as normalizeUpdateChannel, s as checkUpdateStatus, t as runGatewayUpdate, u as resolveNpmChannelTag } from "./update-runner-Dy5Zj6kW.mjs";
import { a as resolveGatewayBindHost, n as isLoopbackHost$1, o as resolveGatewayClientIp, r as isTrustedProxyAddress, s as resolveGatewayListenHosts, t as isLoopbackAddress } from "./net-Bbs_dfMf.mjs";
import { i as resolveGatewayAuth, n as authorizeGatewayConnect, r as isLocalDirectRequest, t as assertGatewayAuthConfigured } from "./auth-CKLbBArb.mjs";
import { i as probeGateway } from "./audit-DMnpd9wB.mjs";
import "./table-DWsrGl_1.mjs";
import { t as buildWorkspaceSkillStatus } from "./skills-status-B4i4oPU3.mjs";
...
import { a as createOutboundSendDeps$1, i as resolveAgentOutboundTarget, r as resolveAgentDeliveryPlan, t as agentCommand } from "./agent-_6k7z_IP.mjs";
import "./daemon-runtime-B-EHQzP_.mjs";
import { n as resolveWideAreaDiscoveryDomain, r as writeWideAreaGatewayZone } from "./widearea-dns-B2Uy-h8T.mjs";
import { i as shouldIncludeHook, n as loadWorkspaceHookEntries, r as resolveHookConfig } from "./hooks-status-DeeGK9W4.mjs";
import "./tui-DDeqszdv.mjs";
import { t as buildChannelAccountSnapshot } from "./status-CnUs4TGe.mjs";
import "./shared-BNu45Bke.mjs";
import { a as runDaemonStop, i as runDaemonStart, n as runDaemonStatus, o as runDaemonUninstall, r as runDaemonRestart, s as runDaemonInstall } from "./daemon-cli-DknrfFYx.mjs";
import { a as toOptionString, i as parsePort$1, n as extractGatewayMiskeys, r as maybeExplainGatewayServiceStop, t as describeUnknownError } from "./shared-DsuUISo3.mjs";
import { i as setGatewayWsLogStyle, n as logWs, r as summarizeAgentEventForWsLog, t as formatForLog } from "./ws-log-6ZqqlMLB.mjs";
import { T as resolveGmailHookRuntimeConfig, _ as buildGogWatchServeArgs, i as ensureTailscaleEndpoint, v as buildGogWatchStartArgs } from "./gmail-setup-utils-Cukne6xz.mjs";
...
function createAgentEventHandler({ broadcast, nodeSendToSession, agentRunSeq, chatRunState, resolveSessionKeyForRun, clearAgentRunContext }) {
	const turnBases = /* @__PURE__ */ new Map();
	const emitChatDelta = (sessionKey, clientRunId, seq, text) => {
		...
		let accumulated;
		if (prevTurnText && text.length < prevTurnText.length && !prevTurnText.startsWith(text)) {
			turnBases.set(clientRunId, prevBuffer);
			accumulated = prevBuffer + "\n\n" + text;
		} else if (base) accumulated = base + "\n\n" + text;
...
async function extractDocumentText(b64, mime, fileName) {
	if (mime === "text/plain" || mime === "text/csv") try {
		return Buffer.from(b64, "base64").toString("utf-8");
	} catch {
		return null;
	}
	if (mime === "application/pdf") try {
		...
		for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
			...
			if (pageText) textParts.push(`--- 第${pageNum}页 ---\n${pageText}`);
		}
...
async function parseMessageWithAttachments(message, attachments, opts) {
	...
	if (documents.length > 0) {
		...
		for (const doc of documents) try {
			...
			} else log?.warn(`attachment ${doc.fileName ?? "document"}: no text extracted`);
		} catch (err) {
			log?.warn(`attachment ${doc.fileName ?? "document"}: text extraction failed: ${String(err)}`);
		}
		if (textBlocks.length > 0) message = message + textBlocks.join("");
	}
	return {
		message,
		images,
...
const chatHandlers = {
	...
	"chat.send": async ({ params, respond, context, client }) => {
		...
		try {
			...
			context.logGateway?.info?.(`chat.send incoming attachments count=${attCount}`);
			if (attCount > 0) {
				const sample = params.attachments[0];
				const sampleInfo = {
					fileName: sample?.fileName ?? sample?.fileName ?? "(unknown)",
					mimeType: typeof sample?.mimeType === "string" ? sample.mimeType : typeof sample?.type === "string" ? sample.type : void 0,
					contentType: typeof sample?.content,
					contentPreviewLength: typeof sample?.content === "string" ? sample.content.length : void 0
				};
				context.logGateway?.info?.(`chat.send attachment[0] summary=${JSON.stringify(sampleInfo)}`);
			}
		} catch (err) {
			context.logGateway?.warn?.(`failed to log incoming attachments: ${String(err)}`);
		}
		...
		if (normalizedAttachments.length > 0) try {
			...
			context.logGateway?.info?.(`parseMessageWithAttachments result: images=${parsedImages.length}, documents=${parsedDocuments.length}`);
		} catch (err) {
			context.logGateway?.warn?.(`parseMessageWithAttachments error: ${String(err)}`);
			respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, String(err)));
			return;
		}
		...
		try {
			...
			let stampedMessage = injectTimestamp(parsedMessage, timestampOptsFromConfig(cfg));
			try {
				if (normalizedAttachments.length > 0) {
					const summaryLines = normalizedAttachments.map((a, i) => {
						return `- ${a.fileName ?? `attachment-${i + 1}`} (${a.mimeType ?? a.type ?? "unknown"})`;
					});
					stampedMessage = `${stampedMessage}\n\n[Attachments]\n${summaryLines.join("\n")}`;
				}
			} catch (err) {
				context.logGateway?.warn?.(`failed to append attachment summary: ${String(err)}`);
			}
			const ctx = {
				Body: parsedMessage,
			...
			try {
				...
				const docCount = parsedDocuments.length;
				const sample = parsedImages[0];
				context.logGateway?.info?.(`dispatchInboundMessage: passing images=${imgCount}, documents=${docCount}` + (sample ? ` sampleMime=${sample.mimeType} sampleBytes=${sample.data?.length}` : ""));
			} catch (err) {
				context.logGateway?.warn?.(`failed to log dispatch images: ${String(err)}`);
			}
			dispatchInboundMessage({
				...
				replyOptions: {
					...
					documents: parsedDocuments.length > 0 ? parsedDocuments : void 0,
...
const nodeHandlers = {
	...
	"node.event": async ({ params, respond, context, client }) => {
		...
		await respondUnavailableOnThrow(respond, async () => {
			const { handleNodeEvent } = await import("./server-node-events-Ba7DZz12.mjs");
...
var WizardSession = class {
	currentStep = null;
	stepDeferred = null;
	answerDeferred = /* @__PURE__ */ new Map();
	status = "running";
	error;
	constructor(runner) {
		this.runner = runner;
...
async function startBrowserControlServerIfEnabled() {
	...
	const mod = override ? await import(override) : await import("./control-service-tDwWDpUV.mjs").then((n) => n.t);
...
async function resolveGatewayRuntimeConfig(params) {
	...
	if (tailscaleMode !== "off" && !isLoopbackHost$1(bindHost)) throw new Error("tailscale serve/funnel requires gateway bind=loopback (127.0.0.1)");
	if (!isLoopbackHost$1(bindHost) && !hasSharedSecret) throw new Error(`refusing to bind gateway to ${bindHost}:${params.port} without auth (set gateway.auth.token/password, or set OPENCLAW_GATEWAY_TOKEN/OPENCLAW_GATEWAY_PASSWORD)`);
	return {
		bindHost,
		controlUiEnabled,
