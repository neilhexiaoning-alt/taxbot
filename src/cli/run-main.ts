...
export async function runCli(argv: string[] = process.argv) {
  ...
  // Skip plugin CLI registration for built-in subcommands — they don't need
  // plugin CLI commands, and loading all plugins via jiti is extremely slow
  // (can take 70+ seconds for TypeScript compilation on first run).
  // Subclis that need plugins (pairing, plugins) load them in their own register().
