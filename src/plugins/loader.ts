...
const resolvePluginSdkAlias = (): string | null => {
  try {
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      const distCandidateJs = path.join(cursor, "dist", "plugin-sdk", "index.js");
      const distCandidateMjs = path.join(cursor, "dist", "plugin-sdk", "index.mjs");
      const orderedCandidates = preferDist
        ? [distCandidateJs, distCandidateMjs, srcCandidate]
        : [srcCandidate, distCandidateJs, distCandidateMjs];
