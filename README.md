...
**基于 OpenClaw 框架的 AI 财税助理 + 智能体市场**

慧助理是一款面向财税人员和普通用户的桌面端 AI 助理应用，集成发票查验、合同税审、纳税申报预审、税务风险治理、票据整理等核心功能，并内置智能体租赁市场，支持发布和使用付费 AI 智能体服务。底层基于 [OpenClaw](https://github.com/openclaw/openclaw) 个人 AI 助理框架，支持通义千问 / MiniMax 等大模型，提供开箱即用的本地化部署方案。首次启动内置配置向导，引导用户完成 API Key 和模型选择，开箱即用。

> 当前版本：**v20260301.4** | 配套市场平台：[慧助理（Skills）自进化平台](https://taxbot.cc) | [TaxSkillStore 源码](https://github.com/neilhexiaoning-alt/TaxSkillStore)
...
### 智能体租赁市场（慧助理 Skills 平台）

- **发布智能体** — 将本地智能体发布到慧助理 Skills 平台，自定义价格和标签
...
- **全屏引导** — 首次启动自动弹出配置向导，三步完成：欢迎 → 配置 → 完成
- **API Key 配置** — 输入 DashScope API Key，支持显示/隐藏切换
- **模型选择** — 通义千问 Plus（推荐）/ Max / Turbo / Long 四种模型一键选择
- **跳过功能** — 用户可跳过向导，稍后在设置中手动配置

### 技能市场

- 浏览和安装慧助理 Skills 平台上的付费技能包
- 支持技能版本更新检查
...
├───────────┬──────────────────────────┤
│  技能系统   │    慧助理 Chat Web UI       │  Lit-html + TypeScript
├───────────┴──────────────────────────┤
│  Qwen Plus / MiniMax M2.5            │  文本推理 + 视觉识别
├──────────────────────────────────────┤
│  慧助理 Skills API (taxbot.cc)       │  技能市场 + 智能体租赁
└──────────────────────────────────────┘
```

- **前端**：Lit 3 + TypeScript，Vite 7 构建，Electron 40 封装为桌面应用
- **后端**：OpenClaw Gateway（Node.js 22+），本地运行，数据不出本机
- **模型**：通义千问 Plus（默认）/ MiniMax M2.5，支持自定义模型和 API 端点
- **技能**：SKILL.md 声明式技能系统，支持预制技能和用户自定义技能
- **知识库**：pdfjs-dist（PDF 提取）、mammoth（DOCX→HTML）、SheetJS（XLSX→HTML）、jszip（PPTX 文本）
- **市场**：[慧助理 Skills 平台](https://taxbot.cc) ([源码](https://github.com/neilhexiaoning-alt/TaxSkillStore)) — Next.js 16 + Prisma + SQLite
...
3. 安装完成后桌面出现慧助理快捷方式，双击启动
4. 首次启动时，内置配置向导引导输入 DashScope API Key 并选择模型
...
│   └── control-ui/        # 慧助理 Chat UI（taxchat.html/js）
├── ui/                    # UI 源码
...
│       ├── rental.ts      # 智能体租赁
│       ├── skills.ts      # 技能管理
│       ├── setup-wizard.ts# 首次启动配置向导
│       ├── taxstore-api.ts# 慧助理 Skills 平台 API 客户端
...
如需修改 API 密钥或模型配置，可直接编辑该文件，或在慧助理设置面板中修改。首次启动时配置向导会自动引导完成基础配置。
...
慧助理的 Electron 主进程提供 40+ 个 IPC 接口：
