...
import { n as registerSubCliByName, t as getSubCliEntries } from "./register.subclis-B8vxNXy3.mjs";
...
async function installCompletion(shell, yes, binName = "openclaw") {
	...
	try {
		...
		const update = updateCompletionProfile(await fsPromises.readFile(profilePath, "utf-8"), binName, cachedPath, sourceLine);
		if (!update.changed) {
