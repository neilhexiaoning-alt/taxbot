...
# Patch package.json: replace source-only build scripts with no-ops
# so that even if setup-taxbot.exe (compiled from old ps1) calls pnpm build, it won't fail
...
if (Test-Path $distPkgJson) {
  ...
  # Replace the build script line with a no-op (echo)
  # Use (?:[^"\\]|\\.)* instead of [^"]*  to handle escaped quotes (e.g. \") in values
  ...
  Write-Info "  Patched package.json build scripts for distribution"
}
Write-OK "Root files"
