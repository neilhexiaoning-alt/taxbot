...
# On Windows (WSL), use node.exe from Windows PATH and convert paths
NODE_CMD="node"
...
INPUT_PATHS=(
  "$(to_windows_path "$ROOT_DIR/package.json")"
  "$(to_windows_path "$ROOT_DIR/pnpm-lock.yaml")"
  "$(to_windows_path "$A2UI_RENDERER_DIR")"
  "$(to_windows_path "$A2UI_APP_DIR")"
)

compute_hash() {
  ROOT_DIR="$(to_windows_path "$ROOT_DIR")" $NODE_CMD --input-type=module - "${INPUT_PATHS[@]}" <<'NODE'
